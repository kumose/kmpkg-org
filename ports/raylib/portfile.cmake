...
file(GLOB vendored_headers RELATIVE "${SOURCE_PATH}/src/external"
    ...
    # Do not use dirent from kmpkg: It is a different implementation which has
...
set(optional_vendored_headers
    "stb_image_resize2.h"  # not yet in kmpkg
)
foreach(header IN LISTS vendored_headers vendored_audio_headers)
    unset(kmpkg_file)
    find_file(kmpkg_file NAMES "${header}" PATHS "${CURRENT_INSTALLED_DIR}/include" PATH_SUFFIXES mmx nanosvg NO_DEFAULT_PATH NO_CACHE)
    if(header IN_LIST vendored_audio_headers AND NOT "audio" IN_LIST FEATURES)
        message(STATUS "Emptying '${header}' (audio disabled)")
        file(WRITE "${SOURCE_PATH}/src/external/${kmpkg_file}" "# audio disabled")
    elseif(kmpkg_file)
        message(STATUS "De-vendoring '${header}'")
        file(COPY "${kmpkg_file}" DESTINATION "${SOURCE_PATH}/src/external")
    elseif(header IN_LIST optional_vendored_headers)
        message(STATUS "Not de-vendoring '${header}' (absent in kmpkg)")
    else()
...
if(KMPKG_LIBRARY_LINKAGE STREQUAL "dynamic")
    kmpkg_replace_string("${CURRENT_PACKAGES_DIR}/include/raylib.h" "defined(USE_LIBTYPE_SHARED)" "1")
endif()
