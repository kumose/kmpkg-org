...
elseif(KMPKG_TARGET_IS_MINGW AND CMAKE_HOST_WIN32)
...
set(PATCHES
    ...
    patches/vulkan-windows.diff        #Forces QMake to use vulkan from kmpkg instead of VULKAN_SDK system variable
...
list(APPEND CORE_OPTIONS
    ...
    -no-angle # Qt does not need to build angle. KMPKG will build angle!
    -no-glib
    -no-feature-gssapi
    )
...
x_kmpkg_pkgconfig_get_modules(PREFIX freetype MODULES freetype2 LIBS)
...
x_kmpkg_pkgconfig_get_modules(PREFIX harfbuzz MODULES harfbuzz LIBS)
if(KMPKG_TARGET_IS_OSX)
...
if(NOT KMPKG_TARGET_IS_WINDOWS)
    ...
    x_kmpkg_pkgconfig_get_modules(PREFIX fontconfig MODULES fontconfig LIBS)
...
if("sqlite3plugin" IN_LIST FEATURES)
    ...
    x_kmpkg_pkgconfig_get_modules(PREFIX sqlite3 MODULES sqlite3 LIBS)
...
if("zstd" IN_LIST FEATURES)
    ...
    x_kmpkg_pkgconfig_get_modules(PREFIX libzstd MODULES libzstd LIBS)
...
if(KMPKG_TARGET_IS_WINDOWS)
    ...
    if(NOT ${KMPKG_LIBRARY_LINKAGE} STREQUAL "static")
...
elseif(KMPKG_TARGET_IS_OSX)
    ...
    if(DEFINED KMPKG_OSX_DEPLOYMENT_TARGET)
        ...
        execute_process(COMMAND sw_vers -productVersion
                OUTPUT_VARIABLE KMPKG_OSX_DEPLOYMENT_TARGET
                OUTPUT_STRIP_TRAILING_WHITESPACE)
        message(STATUS "Detected OSX system Version: ${KMPKG_OSX_DEPLOYMENT_TARGET}")
        string(REGEX MATCH "^([0-9]+)\\.([0-9]+)" KMPKG_OSX_DEPLOYMENT_TARGET "${KMPKG_OSX_DEPLOYMENT_TARGET}")
        message(STATUS "Major.Minor OSX system Version: ${KMPKG_OSX_DEPLOYMENT_TARGET}")
        ...
        # clamp(KMPKG_OSX_DEPLOYMENT_TARGET, QT_MAC_SDK_VERSION_MIN, QT_MAC_SDK_VERSION_MAX)
        if("${KMPKG_OSX_DEPLOYMENT_TARGET}" VERSION_GREATER "${QT_MAC_SDK_VERSION_MAX}")
            set(KMPKG_OSX_DEPLOYMENT_TARGET "${QT_MAC_SDK_VERSION_MAX}")
        endif()
        if("${KMPKG_OSX_DEPLOYMENT_TARGET}" VERSION_LESS "${QT_MAC_SDK_VERSION_MIN}")
            set(KMPKG_OSX_DEPLOYMENT_TARGET "${QT_MAC_SDK_VERSION_MIN}")
        endif()
    ...
    file(READ "${SOURCE_PATH}/mkspecs/common/macx.conf" _tmp_contents)
    string(REPLACE "QMAKE_MACOSX_DEPLOYMENT_TARGET = 10.13" "QMAKE_MACOSX_DEPLOYMENT_TARGET = ${KMPKG_OSX_DEPLOYMENT_TARGET}" _tmp_contents ${_tmp_contents})
    file(WRITE "${SOURCE_PATH}/mkspecs/common/macx.conf" ${_tmp_contents})
...
if(QT_UPDATE_VERSION)
    ...
    #This needs a new KMPKG policy or a static angle build (ANGLE needs to be fixed in KMPKG!)
    if(KMPKG_TARGET_IS_WINDOWS AND ${KMPKG_LIBRARY_LINKAGE} MATCHES "static") # Move angle dll libraries
...
[[    set_target_properties(Qt5::WinMain PROPERTIES
    ...
    if(EXISTS ${CURRENT_PACKAGES_DIR}/tools/qt5/bin)
        ...
        kmpkg_copy_tool_dependencies(${CURRENT_PACKAGES_DIR}/tools/qt5/bin)
    endif()
    ...
    if(EXISTS ${CURRENT_PACKAGES_DIR}/debug/tools/qt5/bin)
        ...
        kmpkg_copy_tool_dependencies(${CURRENT_PACKAGES_DIR}/tools/qt5/debug/bin)
    endif()
