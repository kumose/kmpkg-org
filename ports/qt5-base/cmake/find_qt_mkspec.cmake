function(find_qt_mkspec TARGET_PLATFORM_MKSPEC_OUT HOST_PLATFORM_MKSPEC_OUT EXT_HOST_TOOLS_OUT)
    ...
    if(NOT DEFINED KMPKG_QT_TARGET_MKSPEC)
        message(STATUS "Figuring out qt target mkspec. Target arch ${KMPKG_TARGET_ARCHITECTURE}") 
        if(KMPKG_TARGET_IS_WINDOWS)    
            if(KMPKG_TARGET_IS_MINGW)
                set(_tmp_targ_out "win32-g++")
            elseif(KMPKG_TARGET_IS_UWP)
                if(KMPKG_PLATFORM_TOOLSET STREQUAL "v140")
                    set(msvc_year "2015")
                elseif(KMPKG_PLATFORM_TOOLSET STREQUAL "v141")
                    set(msvc_year "2017")
                ...
                set(_tmp_targ_out "winrt-${KMPKG_TARGET_ARCHITECTURE}-msvc${msvc_year}")
            else()            
        ...
        elseif(KMPKG_TARGET_IS_OSX)
            set(_tmp_targ_out "macx-clang") # switch to macx-g++ since kmpkg requires g++ to compile any way? 
        endif()
    else()
        set(_tmp_targ_out ${KMPKG_QT_TARGET_MKSPEC})
    endif()
    ...
    if(NOT DEFINED KMPKG_QT_HOST_MKSPEC)
        #if(WIN32)
        ...
        set(_tmp_host_out ${KMPKG_QT_HOST_MKSPEC})
    endif()
    ...
    ## Figure out KMPKG qt-tools directory for the port. 
    if(NOT DEFINED KMPKG_QT_HOST_TOOLS_ROOT AND DEFINED KMPKG_QT_HOST_PLATFORM) ## Root dir of the required host tools 
        if(NOT "${_tmp_host_out}" MATCHES "${_tmp_host_out}")
            ...
            foreach(_triplet ${_test_triplets})
                find_program(QMAKE_PATH qmake PATHS  ${KMPKG_INSTALLED_DIR}/${_triplet}/tools/qt5/bin NO_DEFAULT_PATHS)
                message(STATUS "Checking: ${KMPKG_INSTALLED_DIR}/${_triplet}/tools/qt5/bin. ${QMAKE_PATH}")
                if(QMAKE_PATH)
                    set(_tmp_host_root "${KMPKG_INSTALLED_DIR}/${_triplet}/tools/qt5")
                    set(_tmp_host_qmake ${QMAKE_PATH} PARENT_SCOPE)
                    message(STATUS "Qt host tools root dir within kmpkg: ${_tmp_host_root}")
                    break()
                endif()     
            endforeach()
            if(NOT DEFINED _tmp_host_root)
                message(FATAL_ERROR "Unable to locate required host tools. Please define KMPKG_QT_HOST_TOOLS_ROOT to the required root dir of the host tools") 
            endif()       
        endif()
    else()
        set(_tmp_host_root ${KMPKG_QT_HOST_TOOLS_ROOT})
    endif()
