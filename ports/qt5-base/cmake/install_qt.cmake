...
function(install_qt)
    if(CMAKE_HOST_WIN32)
        if(KMPKG_TARGET_IS_MINGW)
            ...
            set(INVOKE "${MINGW32_MAKE}" -j${KMPKG_CONCURRENCY})
        ...
        elseif (KMPKG_QMAKE_USE_NMAKE)
            ...
            set(INVOKE "${JOM}" /J ${KMPKG_CONCURRENCY})
            set(INVOKE_SINGLE "${JOM}" /J 1)
        endif()
    else()
        find_program(MAKE make)
        set(INVOKE "${MAKE}" -j${KMPKG_CONCURRENCY})
        set(INVOKE_SINGLE "${MAKE}" -j1)
    endif()
    kmpkg_find_acquire_program(PYTHON3)
    get_filename_component(PYTHON3_EXE_PATH ${PYTHON3} DIRECTORY)
    kmpkg_add_to_path(PREPEND "${PYTHON3_EXE_PATH}")
   ...
   set(_path "$ENV{PATH}")
    ...
    foreach(_buildname ${BUILDTYPES})
        ...
        set(_installed_libpath_ "${_installed_prefix_}/lib/${KMPKG_HOST_PATH_SEPARATOR}${_installed_prefix_}/lib/manual-link/")

        kmpkg_add_to_path(PREPEND "${_installed_prefix_}/bin")
        kmpkg_add_to_path(PREPEND "${_installed_prefix_}/lib")
        ...
        if(CMAKE_HOST_UNIX)
            if(DEFINED ENV{LD_LIBRARY_PATH})
                ...
                set(ENV{LD_LIBRARY_PATH} "${_installed_libpath_}${KMPKG_HOST_PATH_SEPARATOR}${_ld_library_path_backup_}")
            else()
