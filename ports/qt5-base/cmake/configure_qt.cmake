function(configure_qt)
    ...
    if(DEFINED _csc_HOST_TOOLS_ROOT)
        ## kmpkg internal file struture assumed here!
        message(STATUS "Building Qt with prepared host tools from ${_csc_HOST_TOOLS_ROOT}!")
        kmpkg_add_to_path("${_csc_HOST_TOOLS_ROOT}/bin")
        kmpkg_add_to_path("${_csc_HOST_TOOLS_ROOT}")
    ...
    #Replace with KMPKG variables if PR #7733 is merged
    unset(BUILDTYPES)
    ...
    foreach(_buildname ${BUILDTYPES})
        ...
        if(DEFINED ENV{PKG_CONFIG_PATH})
            ...
            set(ENV{PKG_CONFIG_PATH} "${PKGCONFIG_INSTALLED_DIR}${KMPKG_HOST_PATH_SEPARATOR}${PKGCONFIG_INSTALLED_SHARE_DIR}${KMPKG_HOST_PATH_SEPARATOR}${PKGCONFIG_PACKAGES_DIR}${KMPKG_HOST_PATH_SEPARATOR}${PKGCONFIG_PACKAGES_SHARE_DIR}${KMPKG_HOST_PATH_SEPARATOR}$ENV{PKG_CONFIG_PATH}")
        else()
            set(ENV{PKG_CONFIG_PATH} "${PKGCONFIG_INSTALLED_DIR}${KMPKG_HOST_PATH_SEPARATOR}${PKGCONFIG_INSTALLED_SHARE_DIR}${KMPKG_HOST_PATH_SEPARATOR}${PKGCONFIG_PACKAGES_DIR}${KMPKG_HOST_PATH_SEPARATOR}${PKGCONFIG_PACKAGES_SHARE_DIR}")
        endif()
        ...
        # Note sure about these. KMPKG_QMAKE_OPTIONS offers a way to opt out of these. (earlier values being overwritten by later values; = set +=append *=append unique -=remove)
        ...
        qmake_add_flags("QMAKE_RC" "+=" "${KMPKG_COMBINED_RC_FLAGS_${_buildname}}")
        qmake_add_flags("QMAKE_CFLAGS_${_buildname}" "+=" "${KMPKG_COMBINED_C_FLAGS_${_buildname}}")
        qmake_add_flags("QMAKE_CXXFLAGS_${_buildname}" "+=" "${KMPKG_COMBINED_CXX_FLAGS_${_buildname}}")
        qmake_add_flags("QMAKE_LFLAGS" "+=" "${KMPKG_COMBINED_SHARED_LINKER_FLAGS_${_buildname}}")
        qmake_add_flags("QMAKE_LFLAGS_SHLIB" "+=" "${KMPKG_COMBINED_SHARED_LINKER_FLAGS_${_buildname}}")
        qmake_add_flags("QMAKE_LFLAGS_PLUGIN" "+=" "${KMPKG_COMBINED_MODULE_LINKER_FLAGS_${_buildname}}")
        qmake_add_flags("QMAKE_LIBFLAGS_${_buildname}" "+=" "${KMPKG_COMBINED_STATIC_LINKER_FLAGS_${_buildname}}")
        ...
        set(BUILD_OPTIONS ${_csc_OPTIONS} ${_csc_OPTIONS_${_buildname}}
                ...
                # So with the kmpkg layout there is no way to make it work unless all dll are are copied to tools/qt5/bin and all libs to tools/qt5/lib
