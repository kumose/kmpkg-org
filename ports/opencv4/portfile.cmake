...
kmpkg_from_github(
    ...
    PATCHES
      ...
      0022-android-use-kmpkg-cpu-features.patch
      0022-fix-miss-exception-include.patch
)

kmpkg_find_acquire_program(PKGCONFIG)
set(ENV{PKG_CONFIG} "${PKGCONFIG}")
kmpkg_host_path_list(APPEND ENV{PKG_CONFIG_PATH} "${CURRENT_INSTALLED_DIR}/lib/pkgconfig")
...
string(COMPARE EQUAL "${KMPKG_CRT_LINKAGE}" "static" BUILD_WITH_STATIC_CRT)
...
# Cannot use kmpkg_check_features() for "qt" because it requires the QT version number passed, not just a boolean
...
if("dnn" IN_LIST FEATURES)
  set(FLATC "${CURRENT_HOST_INSTALLED_DIR}/tools/flatbuffers/flatc${KMPKG_HOST_EXECUTABLE_SUFFIX}")
...
if("python" IN_LIST FEATURES)
  x_kmpkg_get_python_packages(PYTHON_VERSION "3" PACKAGES numpy OUT_PYTHON_VAR "PYTHON3")
...
if("dnn" IN_LIST FEATURES)
  kmpkg_download_distfile(TINYDNN_ARCHIVE
...
if("cuda" IN_LIST FEATURES)
  kmpkg_download_distfile(OCV_DOWNLOAD
...
if(KMPKG_TARGET_IS_ANDROID AND (KMPKG_TARGET_ARCHITECTURE MATCHES "^arm"))
  kmpkg_download_distfile(OCV_DOWNLOAD
...
if (NOT KMPKG_BUILD_TYPE)
  ...
  kmpkg_replace_string("${CURRENT_PACKAGES_DIR}/share/opencv4/OpenCVModules-debug.cmake"
      "\${_IMPORT_PREFIX}/sdk"
...
if("contrib" IN_LIST FEATURES AND NOT KMPKG_TARGET_IS_UWP AND NOT KMPKG_TARGET_IS_IOS AND NOT (KMPKG_TARGET_IS_WINDOWS AND KMPKG_TARGET_ARCHITECTURE MATCHES "^arm"))
  string(APPEND DEPS_STRING "
...
foreach(PYTHON3_SITE_FILE ${PYTHON3_SITE_FILES})
  kmpkg_replace_string("${PYTHON3_SITE_FILE}"
...
foreach(PYTHON3_SITE_FILE_DEBUG ${PYTHON3_SITE_FILES_DEBUG})
  kmpkg_replace_string("${PYTHON3_SITE_FILE_DEBUG}"
...
if (EXISTS "${CURRENT_PACKAGES_DIR}/lib/pkgconfig/opencv4.pc")
  kmpkg_replace_string("${CURRENT_PACKAGES_DIR}/lib/pkgconfig/opencv4.pc"
    "-lQt6::Core5Compat"
    "-lQt6Core5Compat"
    IGNORE_UNCHANGED
  )
...
if (EXISTS "${CURRENT_PACKAGES_DIR}/debug/lib/pkgconfig/opencv4.pc")
  kmpkg_replace_string("${CURRENT_PACKAGES_DIR}/debug/lib/pkgconfig/opencv4.pc"
    "-lQt6::Core5Compat"
    "-lQt6Core5Compat"
    IGNORE_UNCHANGED
  )
...
kmpkg_install_copyright(FILE_LIST "${SOURCE_PATH}/LICENSE" ${extra1_license_files} ${extra2_license_files})
