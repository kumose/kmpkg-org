...
if(KMPKG_DETECTED_MSVC)
    ...
    kmpkg_install_nmake(SOURCE_PATH "${SOURCE_PATH}"
    ...
    if (KMPKG_LIBRARY_LINKAGE STREQUAL "dynamic")
        kmpkg_replace_string("${CURRENT_PACKAGES_DIR}/include/luajit/luaconf.h" "defined(LUA_BUILD_AS_DLL)" "1")
    endif()
    ...
    if(KMPKG_CROSSCOMPILING)
        list(APPEND options
            "LJARCH=${KMPKG_TARGET_ARCHITECTURE}"
            "BUILDVM_X=${CURRENT_HOST_INSTALLED_DIR}/manual-tools/${PORT}/buildvm-${KMPKG_TARGET_ARCHITECTURE}${KMPKG_HOST_EXECUTABLE_SUFFIX}"
        )
    endif()

    kmpkg_list(SET make_options "EXECUTABLE_SUFFIX=${KMPKG_TARGET_EXECUTABLE_SUFFIX}")
    set(strip_options "") # cf. src/Makefile
    if(KMPKG_TARGET_IS_OSX)
        kmpkg_list(APPEND make_options "TARGET_SYS=Darwin")
        set(strip_options " -x")
    elseif(KMPKG_TARGET_IS_IOS)
        kmpkg_list(APPEND make_options "TARGET_SYS=iOS")
        set(strip_options " -x")
    elseif(KMPKG_TARGET_IS_LINUX)
        kmpkg_list(APPEND make_options "TARGET_SYS=Linux")
    elseif(KMPKG_TARGET_IS_WINDOWS)
        kmpkg_list(APPEND make_options "TARGET_SYS=Windows")
    ...
    kmpkg_configure_make(SOURCE_PATH "${SOURCE_PATH}"
        COPY_SOURCE
        OPTIONS
            "BUILDMODE=${KMPKG_LIBRARY_LINKAGE}"
            ${options}
        OPTIONS_RELEASE
            "DASM_ARCHS=${dasm_archs}"
    )
    kmpkg_install_make(
        MAKEFILE "Makefile.kmpkg"
        OPTIONS
            ${make_options}
            "TARGET_AR=${KMPKG_DETECTED_CMAKE_AR} rcus"
            "TARGET_STRIP=${KMPKG_DETECTED_CMAKE_STRIP}${strip_options}"
    )
endif()
...
kmpkg_copy_tools(TOOL_NAMES luajit AUTO_CLEAN)
