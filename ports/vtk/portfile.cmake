...
set(KMPKG_POLICY_SKIP_ABSOLUTE_PATHS_CHECK enabled)
...
kmpkg_from_github(
    ...
    PATCHES
        ...
        FindExpat.patch # The find_library calls are taken care of by kmpkg-cmake-wrapper.cmake of expat
        # fix-gdal.patch TODO?
        cgns.patch
        vtkm.patch
        afxdll.patch
...
kmpkg_replace_string("${SOURCE_PATH}/ThirdParty/ogg/CMakeLists.txt" "OGG::OGG" "Ogg::ogg")
kmpkg_replace_string("${SOURCE_PATH}/ThirdParty/ogg/CMakeLists.txt" "OGG" "Ogg")
kmpkg_replace_string("${SOURCE_PATH}/CMake/vtkInstallCMakePackage.cmake" "FindOGG.cmake\n" "")
kmpkg_replace_string("${SOURCE_PATH}/CMake/FindTHEORA.cmake" "find_dependency(OGG)" "find_dependency(Ogg CONFIG)")
kmpkg_replace_string("${SOURCE_PATH}/CMake/FindTHEORA.cmake" "OGG::OGG" "Ogg::ogg")
...
kmpkg_check_features(OUT_FEATURE_OPTIONS VTK_YES_NO_OPTIONS
    FEATURES
        "all"         VTK_BUILD_ALL_MODULES
        "atlmfc"      VTK_MODULE_ENABLE_VTK_GUISupportMFC
        "cgns"        KMPKG_LOCK_FIND_PACKAGE_CGNS
        "cuda"        VTK_USE_CUDA
        "debugleaks"  VTK_DEBUG_LEAKS
        "fontconfig"  VTK_MODULE_ENABLE_VTK_RenderingFreeTypeFontConfig
        "libharu"     KMPKG_LOCK_FIND_PACKAGE_LibHaru
        "libtheora"   KMPKG_LOCK_FIND_PACKAGE_THEORA
        "netcdf"      KMPKG_LOCK_FIND_PACKAGE_NetCDF
        ...
        "proj"        KMPKG_LOCK_FIND_PACKAGE_PROJ
        ...
        "seacas"      KMPKG_LOCK_FIND_PACKAGE_SEACASExodus
        "seacas"      KMPKG_LOCK_FIND_PACKAGE_SEACASIoss
        "sql"         KMPKG_LOCK_FIND_PACKAGE_SQLite3
...
kmpkg_check_features(OUT_FEATURE_OPTIONS VTK_FEATURE_OPTIONS
    FEATURES
...
if("qt" IN_LIST FEATURES)
    file(READ "${CURRENT_INSTALLED_DIR}/share/qtbase/kmpkg_abi_info.txt" qtbase_abi_info)
    ...
    if(NOT EXISTS "${CURRENT_HOST_INSTALLED_DIR}/tools/Qt6/bin/qmlplugindump${KMPKG_HOST_EXECUTABLE_SUFFIX}")
...
if("python" IN_LIST FEATURES)
    ...
    list(APPEND ADDITIONAL_OPTIONS
        ...
        "-DVTK_PYTHON_SITE_PACKAGES_SUFFIX=${PYTHON3_SITE}" # from kmpkg-port-config.cmake
    )
...
kmpkg_cmake_configure(
    ...
    OPTIONS
        ...
        # Disable dependencies which are not in kmpkg or not in the manifest
        -DKMPKG_LOCK_FIND_PACKAGE_Boost=OFF
        -DVTK_ENABLE_OSPRAY=OFF
        -DVTK_USE_TK=OFF # TCL/TK currently not included in kmpkg
        ...
        -DVTK_MODULE_USE_EXTERNAL_VTK_token:BOOL=OFF # Not yet in KMPKG
        # misc
        ...
        -DKMPKG_HOST_TRIPLET=${_HOST_TRIPLET}
    ...
    MAYBE_UNUSED_VARIABLES
        ...
        KMPKG_LOCK_FIND_PACKAGE_Boost
        KMPKG_LOCK_FIND_PACKAGE_CGNS
        KMPKG_LOCK_FIND_PACKAGE_LibHaru
        KMPKG_LOCK_FIND_PACKAGE_NetCDF
        KMPKG_LOCK_FIND_PACKAGE_PROJ
        KMPKG_LOCK_FIND_PACKAGE_SEACASExodus
        KMPKG_LOCK_FIND_PACKAGE_SEACASIoss
        KMPKG_LOCK_FIND_PACKAGE_SQLite3
        KMPKG_LOCK_FIND_PACKAGE_THEORA
)
...
# =============================================================================
# Fixup target files
kmpkg_cmake_config_fixup(CONFIG_PATH lib/cmake/vtk-${VTK_SHORT_VERSION})
...
function(_vtk_remove_debug_tool TOOL_NAME)
    set(filename "${CURRENT_PACKAGES_DIR}/debug/bin/${TOOL_NAME}${KMPKG_TARGET_EXECUTABLE_SUFFIX}")
    if(EXISTS "${filename}")
        file(REMOVE "${filename}")
    endif()
    set(filename "${CURRENT_PACKAGES_DIR}/debug/bin/${TOOL_NAME}d${KMPKG_TARGET_EXECUTABLE_SUFFIX}")
    if(EXISTS "${filename}")
        file(REMOVE "${filename}")
    endif()
...
function(_vtk_move_release_tool TOOL_NAME)
    set(old_filename "${CURRENT_PACKAGES_DIR}/bin/${TOOL_NAME}${KMPKG_TARGET_EXECUTABLE_SUFFIX}")
...
# TODO: Replace with kmpkg_copy_tools if known which tools are built with which feature
# or add and option to kmpkg_copy_tools which does not require the tool to be present
foreach(TOOL_NAME IN LISTS VTK_TOOLS)
    _vtk_remove_debug_tool("${TOOL_NAME}")
    _vtk_move_release_tool("${TOOL_NAME}")
endforeach()

if(EXISTS "${CURRENT_PACKAGES_DIR}/bin/vtktoken-9.3.dll" AND KMPKG_LIBRARY_LINKAGE STREQUAL "static")
  # vendored "token" library can be only build as a shared library
  set(KMPKG_POLICY_DLLS_IN_STATIC_LIBRARY enabled)
...
kmpkg_copy_tool_dependencies("${CURRENT_PACKAGES_DIR}/tools/vtk")
...
# Use kmpkg provided find method
...
if(EXISTS "${CURRENT_PACKAGES_DIR}/include/vtk-${VTK_SHORT_VERSION}/vtkChemistryConfigure.h")
    kmpkg_replace_string("${CURRENT_PACKAGES_DIR}/include/vtk-${VTK_SHORT_VERSION}/vtkChemistryConfigure.h" "${SOURCE_PATH}" "not/existing" IGNORE_UNCHANGED)
endif()

kmpkg_replace_string("${CURRENT_PACKAGES_DIR}/share/vtk/VTK-vtk-module-properties.cmake" "_vtk_module_import_prefix}/lib/vtk-9.3/hierarchy" "_vtk_module_import_prefix}$<$<CONFIG:Debug>:/debug>/lib/vtk-9.3/hierarchy")

file(COPY "${CMAKE_CURRENT_LIST_DIR}/usage" DESTINATION "${CURRENT_PACKAGES_DIR}/share/${PORT}")

file(RENAME "${CURRENT_PACKAGES_DIR}/share/licenses" "${CURRENT_PACKAGES_DIR}/share/${PORT}/licenses")
kmpkg_install_copyright(FILE_LIST "${SOURCE_PATH}/Copyright.txt" COMMENT [[
