...
kmpkg_download_distfile(PYTHON_GPU_WRAPPING_PATCH
...
kmpkg_from_github(
    ...
    #[[
        ...
        Try `kmpkg install itk[core,cuda,rtk] --only-downloads` for suggestions and verification.
    #]]
...
if("vtk" IN_LIST FEATURES AND EXISTS "${CURRENT_INSTALLED_DIR}/share/vtk/VTKPython-targets.cmake")
    ...
    include("${CURRENT_INSTALLED_DIR}/share/python3/kmpkg-port-config.cmake")
...
if("python" IN_LIST FEATURES)
    ...
    list(APPEND ADDITIONAL_OPTIONS
        ...
        "-DCASTXML_EXECUTABLE=${CURRENT_HOST_INSTALLED_DIR}/tools/castxml/bin/castxml${KMPKG_HOST_EXECUTABLE_SUFFIX}"
        -DPython3_FIND_REGISTRY=NEVER
        "-DPython3_EXECUTABLE:PATH=${PYTHON3}" # Required by more than one feature
    )
    #ITK_PYTHON_SITE_PACKAGES_SUFFIX should be set to the install dir of the site-packages within kmpkg

    kmpkg_find_acquire_program(SWIG) # Swig is only required for wrapping!
...
if (KMPKG_TARGET_ARCHITECTURE STREQUAL x64 OR KMPKG_TARGET_ARCHITECTURE STREQUAL arm64)
...
kmpkg_cmake_configure(
    ...
    OPTIONS
        ...
        #-DITK_USE_SYSTEM_LIBRARIES=ON # enables USE_SYSTEM for all third party libraries, some of which do not have kmpkg ports such as CastXML, SWIG, MINC etc
        ...
        -DITK_USE_SYSTEM_OpenJPEG=ON # Added by KMPKG
        -DITK_USE_SYSTEM_VXL=ON
        #-DITK_USE_SYSTEM_CASTXML=ON # needs to be added to kmpkg_find_acquire_program https://data.kitware.com/api/v1/file/hashsum/sha512/b8b6f0aff11fe89ab2fcd1949cc75f2c2378a7bc408827a004396deb5ff5a9976bffe8a597f8db1b74c886ea39eb905e610dce8f5bd7586a4d6c196d7349da8d/download
...
if("rtk" IN_LIST FEATURES)
    kmpkg_replace_string("${CURRENT_PACKAGES_DIR}/include/ITK-5.4/rtkConfiguration.h" "#define RTK_BINARY_DIR \"${CURRENT_BUILDTREES_DIR}/${TARGET_TRIPLET}-rel/Modules/Remote/RTK\"" "")
    kmpkg_replace_string("${CURRENT_PACKAGES_DIR}/include/ITK-5.4/rtkConfiguration.h" "#define RTK_DATA_ROOT \"${CURRENT_BUILDTREES_DIR}/${TARGET_TRIPLET}-rel/ExternalData/Modules/Remote/RTK/test\"" "")
endif()

kmpkg_list(SET file_list
    "${SOURCE_PATH}/NOTICE"
    "${SOURCE_PATH}/LICENSE"
)
if("cuda" IN_LIST FEATURES)
    kmpkg_list(APPEND file_list
...
kmpkg_install_copyright(FILE_LIST ${file_list})
