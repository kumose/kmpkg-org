...
kmpkg_add_to_path(PREPEND "${BAZEL_DIR}")
set(ENV{BAZEL_BIN_PATH} "${BAZEL_DIR}")

function(tensorflow_try_remove_recurse_wait PATH_TO_REMOVE)
	file(REMOVE_RECURSE ${PATH_TO_REMOVE})
	if(EXISTS "${PATH_TO_REMOVE}")
		kmpkg_execute_required_process(COMMAND ${CMAKE_COMMAND} -E sleep 5 WORKING_DIRECTORY ${CURRENT_BUILDTREES_DIR} LOGNAME prerequisites-sleep-${TARGET_TRIPLET})
		file(REMOVE_RECURSE ${PATH_TO_REMOVE})
	endif()
endfunction()
...
kmpkg_add_to_path(PREPEND ${GIT_DIR})

string(FIND "${CURRENT_BUILDTREES_DIR}" " " POS)
if(NOT POS EQUAL -1)
	message(FATAL_ERROR "Your kmpkg path contains spaces. This is not supported by the bazel build tool. Aborting.")
endif()
...
if(CMAKE_HOST_WIN32)
	...
	kmpkg_acquire_msys(MSYS_ROOT PACKAGES bash unzip patch diffutils libintl gzip coreutils mingw-w64-x86_64-python-numpy)
	kmpkg_add_to_path(${MSYS_ROOT}/usr/bin)
	kmpkg_add_to_path(${MSYS_ROOT}/mingw64/bin)
	...
	if (KMPKG_HOST_IS_OSX)
		...
		if(HOST_ARCH STREQUAL "arm64")
			...
			if (NOT EXISTS ${CURRENT_BUILDTREES_DIR}/miniforge3)
				kmpkg_execute_required_process(COMMAND curl -fsSLo Miniforge3.sh "https://github.com/conda-forge/miniforge/releases/latest/download/Miniforge3-MacOSX-arm64.sh" WORKING_DIRECTORY ${CURRENT_BUILDTREES_DIR} LOGNAME prerequisites-miniforge3-${TARGET_TRIPLET})
				kmpkg_execute_required_process(COMMAND bash ./Miniforge3.sh -p ${CURRENT_BUILDTREES_DIR}/miniforge3 -b WORKING_DIRECTORY ${CURRENT_BUILDTREES_DIR} LOGNAME prerequisites-miniforge3-${TARGET_TRIPLET})
				SET(PYTHON3 ${CURRENT_BUILDTREES_DIR}/miniforge3/bin/python3)
			endif()
		endif()
	endif()
	kmpkg_execute_required_process(COMMAND ${PYTHON3} -m venv --symlinks "${CURRENT_BUILDTREES_DIR}/${TARGET_TRIPLET}-venv"  WORKING_DIRECTORY ${CURRENT_BUILDTREES_DIR} LOGNAME prerequisites-venv-${TARGET_TRIPLET})
	kmpkg_add_to_path(PREPEND ${CURRENT_BUILDTREES_DIR}/${TARGET_TRIPLET}-venv/bin)
	...
	if(KMPKG_TARGET_IS_OSX)
		kmpkg_execute_required_process(COMMAND ${PYTHON3} -m pip install -U pip WORKING_DIRECTORY ${CURRENT_BUILDTREES_DIR} LOGNAME prerequisites-pip-${TARGET_TRIPLET})
		...
		set(ENV{LAPACK} "None")
		set(ENV{ATLAS} "None")
		kmpkg_execute_required_process(COMMAND ${PYTHON3} -m pip install -U --force-reinstall numpy setuptools WORKING_DIRECTORY ${CURRENT_BUILDTREES_DIR} LOGNAME prerequistes-pip-${TARGET_TRIPLET})
	else()
		kmpkg_execute_required_process(COMMAND ${PYTHON3} -m pip install -U pip numpy WORKING_DIRECTORY ${CURRENT_BUILDTREES_DIR} LOGNAME prerequisites-pip-${TARGET_TRIPLET})
	endif()
endif()
...
kmpkg_execute_required_process(COMMAND ${PYTHON3} -c "import numpy" WORKING_DIRECTORY ${CURRENT_BUILDTREES_DIR} LOGNAME prerequisites-numpy-${TARGET_TRIPLET})
...
foreach(BUILD_TYPE IN LISTS PORT_BUILD_CONFIGS)
	...
	if(DEFINED ENV{BAZEL_CUSTOM_CACERTS})
		...
		if(DEFINED ENV{HTTPS_PROXY})
			message(STATUS "You are using HTTPS_PROXY. In case you encounter bazel certificate errors, you might want to set: BAZEL_CUSTOM_CACERTS=/path/to/trust.store (and optionally BAZEL_CUSTOM_CACERTS_PASSWORD), and to enable kmpkg to actually use it: KMPKG_KEEP_ENV_VARS=BAZEL_CUSTOM_CACERTS;BAZEL_CUSTOM_CACERTS_PASSWORD")
			if(CMAKE_HOST_WIN32)
	...
	set(CXXOPTS)
	set(LINKOPTS)
	set(BUILD_OPTS --jobs ${KMPKG_CONCURRENCY})
	message(STATUS "Build Tensorflow with concurrent level: ${KMPKG_CONCURRENCY}")
	if(KMPKG_TARGET_IS_WINDOWS)
	...
	if(BUILD_TYPE STREQUAL dbg)
		...
		separate_arguments(KMPKG_C_FLAGS ${PLATFORM_COMMAND} ${KMPKG_C_FLAGS})
		separate_arguments(KMPKG_C_FLAGS_DEBUG ${PLATFORM_COMMAND} ${KMPKG_C_FLAGS_DEBUG})
		foreach(OPT IN LISTS KMPKG_C_FLAGS KMPKG_C_FLAGS_DEBUG)
			list(APPEND COPTS "--copt=${OPT}")
		endforeach()
		separate_arguments(KMPKG_CXX_FLAGS ${PLATFORM_COMMAND} ${KMPKG_CXX_FLAGS})
		separate_arguments(KMPKG_CXX_FLAGS_DEBUG ${PLATFORM_COMMAND} ${KMPKG_CXX_FLAGS_DEBUG})
		foreach(OPT IN LISTS KMPKG_CXX_FLAGS KMPKG_CXX_FLAGS_DEBUG)
			list(APPEND CXXOPTS "--cxxopt=${OPT}")
		endforeach()
		separate_arguments(KMPKG_LINKER_FLAGS ${PLATFORM_COMMAND} ${KMPKG_LINKER_FLAGS})
		separate_arguments(KMPKG_LINKER_FLAGS_DEBUG ${PLATFORM_COMMAND} ${KMPKG_LINKER_FLAGS_DEBUG})
		foreach(OPT IN LISTS KMPKG_LINKER_FLAGS KMPKG_LINKER_FLAGS_DEBUG)
		...
		separate_arguments(KMPKG_C_FLAGS ${PLATFORM_COMMAND} ${KMPKG_C_FLAGS})
		separate_arguments(KMPKG_C_FLAGS_RELEASE ${PLATFORM_COMMAND} ${KMPKG_C_FLAGS_RELEASE})
		foreach(OPT IN LISTS KMPKG_C_FLAGS KMPKG_C_FLAGS_RELEASE)
			list(APPEND COPTS "--copt=${OPT}")
		endforeach()
		separate_arguments(KMPKG_CXX_FLAGS ${PLATFORM_COMMAND} ${KMPKG_CXX_FLAGS})
		separate_arguments(KMPKG_CXX_FLAGS_RELEASE ${PLATFORM_COMMAND} ${KMPKG_CXX_FLAGS_RELEASE})
		foreach(OPT IN LISTS KMPKG_CXX_FLAGS KMPKG_CXX_FLAGS_RELEASE)
			list(APPEND CXXOPTS "--cxxopt=${OPT}")
		endforeach()
		separate_arguments(KMPKG_LINKER_FLAGS ${PLATFORM_COMMAND} ${KMPKG_LINKER_FLAGS})
		separate_arguments(KMPKG_LINKER_FLAGS_RELEASE ${PLATFORM_COMMAND} ${KMPKG_LINKER_FLAGS_RELEASE})
		foreach(OPT IN LISTS KMPKG_LINKER_FLAGS KMPKG_LINKER_FLAGS_RELEASE)
			list(APPEND LINKOPTS "--linkopt=${OPT}")
		endforeach()
	endif()

	if(KMPKG_TARGET_IS_OSX AND KMPKG_TARGET_ARCHITECTURE STREQUAL "arm64")
...
# NOTE: if this port ever supports KMPKG_BUILD_TYPE, use that to set these.
...
if(KMPKG_TARGET_IS_WINDOWS)
	if(KMPKG_LIBRARY_LINKAGE STREQUAL dynamic)
		...
		set(KMPKG_POLICY_MISMATCHED_NUMBER_OF_BINARIES enabled)
