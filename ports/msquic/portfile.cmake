...
if(KMPKG_HOST_IS_WINDOWS)
    ...
    kmpkg_add_to_path("${jom_dir}")
else()
    find_program(MAKE make)
    cmake_path(GET MAKE PARENT_PATH make_dir)
    kmpkg_add_to_path("${make_dir}")
endif()

if(KMPKG_TARGET_IS_WINDOWS)
    kmpkg_find_acquire_program(NASM)
    cmake_path(GET NASM PARENT_PATH nasm_dir)
    kmpkg_add_to_path("${nasm_dir}")
endif()

string(COMPARE EQUAL "${KMPKG_LIBRARY_LINKAGE}" "dynamic" QUIC_BUILD_SHARED)
...
kmpkg_cmake_configure(
    ...
    OPTIONS
        ...
        "-DQUIC_UWP_BUILD=${KMPKG_TARGET_IS_UWP}"
)
...
if(platform)
    kmpkg_replace_string("${CURRENT_PACKAGES_DIR}/include/quic_platform.h"
        "#elif ${platform}"
        "#elif 1
#ifndef ${platform}
#define ${platform}
...
kmpkg_install_copyright(FILE_LIST "${QUIC_SOURCE_PATH}/LICENSE" "${QUIC_SOURCE_PATH}/THIRD-PARTY-NOTICES")
