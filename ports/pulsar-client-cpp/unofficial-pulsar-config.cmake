if (NOT TARGET unofficial::pulsar::pulsar)
    get_filename_component(KMPKG_IMPORT_PREFIX "${CMAKE_CURRENT_LIST_FILE}" PATH)
    get_filename_component(KMPKG_IMPORT_PREFIX "${KMPKG_IMPORT_PREFIX}" PATH)
    get_filename_component(KMPKG_IMPORT_PREFIX "${KMPKG_IMPORT_PREFIX}" PATH)

    find_path(_pulsar_include_dir NAMES "pulsar/Client.h" PATHS "${KMPKG_IMPORT_PREFIX}/include" NO_DEFAULT_PATH)
    find_library(_pulsar_library_release NAMES pulsar pulsar-static PATHS "${KMPKG_IMPORT_PREFIX}/lib" NO_DEFAULT_PATH)
    find_library(_pulsar_library_debug NAMES pulsar pulsar-static PATHS "${KMPKG_IMPORT_PREFIX}/debug/lib" NO_DEFAULT_PATH)
    ...
    if (NOT _pulsar_include_dir OR NOT _pulsar_library_release)
        message(FATAL_ERROR "Broken installation of kmpkg port pulsar-client-cpp")
    endif ()

    if (MSVC AND "@KMPKG_LIBRARY_LINKAGE@" STREQUAL "dynamic")
        find_file(_pulsar_release_dll NAMES "pulsar.dll" PATHS "${KMPKG_IMPORT_PREFIX}/bin" NO_DEFAULT_PATH)
        find_file(_pulsar_debug_dll NAMES "pulsar.dll" PATHS "${KMPKG_IMPORT_PREFIX}/debug/bin" NO_DEFAULT_PATH)
