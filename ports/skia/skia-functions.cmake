# Declare a named external dependency for download with kmpkg_from_git,
...
# Declare a named external dependencies to be resolved via kmpkg installed tree.
function(declare_external_from_kmpkg name)
    set(skia_external_kmpkg_${name} "${ARGN}" PARENT_SCOPE)
endfunction()

# Download and integrate named external dependencies.
# Downlods must be handled before kmpkg in order to support --only-downloads mode.
function(get_externals)
    ...
    foreach(name IN LISTS ARGN)
        ...
        elseif(DEFINED "skia_external_kmpkg_${name}")
            list(APPEND from_kmpkg "${name}")
        else()
    ...
    endforeach()
    foreach(name IN LISTS from_kmpkg)
        third_party_from_kmpkg("${name}" ${skia_external_kmpkg_${name}})
    endforeach()
endfunction()
...
function(third_party_from_pkgconfig gn_group)
    ...
    x_kmpkg_pkgconfig_get_modules(PREFIX PC_${module} MODULES ${arg_MODULES} CFLAGS LIBS)
...
# Setup a third-party dependency from kmpkg installed tree
function(third_party_from_kmpkg gn_group)
...
function(expand_gn_targets_recurse gn_target desc_var source_path)
    ...
    if(target_type STREQUAL "static_library" AND KMPKG_LIBRARY_LINKAGE STREQUAL "static")
...
function(get_link_libs out_var desc_json target)
    ...
    separate_arguments(known_standard_libraries NATIVE_COMMAND "${KMPKG_DETECTED_CMAKE_CXX_STANDARD_LIBRARIES}")
    ...
    foreach(lib IN LISTS frameworks ldflags libs)
        ...
        string(REPLACE "${CURRENT_INSTALLED_DIR}" "\${z_kmpkg_${PORT}_root}" lib "${lib}")
        string(REPLACE "${CURRENT_PACKAGES_DIR}" "\${z_kmpkg_${PORT}_root}" lib "${lib}")
        if(NOT lib MATCHES "^-L")
            kmpkg_list(REMOVE_ITEM output "${lib}")
        endif()
        kmpkg_list(APPEND output "${lib}")
    endforeach()
    set("${out_var}" "${output}" PARENT_SCOPE)
endfunction()

# A revised variant of kmpkg_gn_install
function(skia_gn_install_build_type)
    ...
    # build
    set(KMPKG_BUILD_TYPE "${arg_BUILD_TYPE}")
    kmpkg_build_ninja(TARGETS ${arg_TARGETS})
...
function(skia_gn_install)
    ...
    if(DEFINED arg_UNPARSED_ARGUMENTS)
        message(WARNING "kmpkg_gn_install was passed extra arguments: ${arg_UNPARSED_ARGUMENTS}")
    endif()
