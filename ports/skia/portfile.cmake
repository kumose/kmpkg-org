...
# these following aren't available in kmpkg
...
declare_external_from_kmpkg(vulkan_headers PATH third_party/externals/vulkan-headers)

set(known_cpus x86 x64 arm arm64 wasm)
if(NOT KMPKG_TARGET_ARCHITECTURE IN_LIST known_cpus)
    message(WARNING "Unknown target cpu '${KMPKG_TARGET_ARCHITECTURE}'.")
endif()

string(JOIN " " OPTIONS
    "target_cpu=\"${KMPKG_TARGET_ARCHITECTURE}\""
...
kmpkg_list(SET SKIA_TARGETS :skia :modules)
...
elseif(KMPKG_TARGET_IS_LINUX OR KMPKG_TARGET_IS_BSD)
...
if (KMPKG_TARGET_IS_BSD AND NOT KMPKG_TARGET_IS_FREEBSD)
    string(APPEND OPTIONS " skia_kmpkg_has_lib_dl=false")
else()
    string(APPEND OPTIONS " skia_kmpkg_has_lib_dl=true")
endif()
...
if("gl" IN_LIST FEATURES)
    if (KMPKG_TARGET_IS_LINUX OR KMPKG_TARGET_IS_BSD)
...
if("icu" IN_LIST FEATURES)
    kmpkg_replace_string("${SOURCE_PATH}/third_party/icu/BUILD.gn"
        [[config("kmpkg_icu") {]]
        [[import("icu.gni")
config("kmpkg_icu")  {]])
endif()
...
kmpkg_replace_string("${SOURCE_PATH}/.gn" "script_executable = \"python3\"" "script_executable = \"${PYTHON3}\"")
kmpkg_replace_string("${SOURCE_PATH}/gn/toolchain/BUILD.gn" "python3 " "\\\"${PYTHON3}\\\" ")
...
elseif(KMPKG_TARGET_IS_ANDROID)
    string(APPEND OPTIONS " ndk=\"${KMPKG_DETECTED_CMAKE_ANDROID_NDK}\" ndk_api=${KMPKG_DETECTED_CMAKE_SYSTEM_VERSION}")
else()
    string(APPEND OPTIONS " \
        cc=\"${KMPKG_DETECTED_CMAKE_C_COMPILER}\" \
        cxx=\"${KMPKG_DETECTED_CMAKE_CXX_COMPILER}\"")
endif()

string_to_gn_list(SKIA_C_FLAGS_DBG "${KMPKG_COMBINED_C_FLAGS_DEBUG}")
string_to_gn_list(SKIA_CXX_FLAGS_DBG "${KMPKG_COMBINED_CXX_FLAGS_DEBUG}")
string(APPEND OPTIONS_DBG " \
    extra_cflags_c=${SKIA_C_FLAGS_DBG} \
    extra_cflags_cc=${SKIA_CXX_FLAGS_DBG}")
string_to_gn_list(SKIA_C_FLAGS_REL "${KMPKG_COMBINED_C_FLAGS_RELEASE}")
string_to_gn_list(SKIA_CXX_FLAGS_REL "${KMPKG_COMBINED_CXX_FLAGS_RELEASE}")
...
kmpkg_replace_string("${CURRENT_PACKAGES_DIR}/include/skia/include/private/base/SkAPI.h" "defined(SKIA_DLL)" "${skia_dll_${KMPKG_LIBRARY_LINKAGE}}")

# kmpkg legacy layout omits "include/" component. Just duplicate.
...
// Test for https://github.com/microsoft/kmpkg/issues/27219
...
kmpkg_install_copyright(FILE_LIST "${SOURCE_PATH}/LICENSE" ${third_party_licenses})
