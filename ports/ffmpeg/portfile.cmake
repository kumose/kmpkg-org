...
elseif(KMPKG_CMAKE_SYSTEM_NAME STREQUAL "QNX")
    string(APPEND OPTIONS " --target-os=qnx")
endif()

if(KMPKG_TARGET_IS_OSX)
    list(JOIN KMPKG_OSX_ARCHITECTURES " " OSX_ARCHS)
    list(LENGTH KMPKG_OSX_ARCHITECTURES OSX_ARCH_COUNT)
endif()
...
if(KMPKG_DETECTED_MSVC)
    ...
    string(APPEND KMPKG_COMBINED_C_FLAGS_DEBUG " -O2")
    string(REGEX REPLACE "(^| )-RTC1( |$)" " " KMPKG_COMBINED_C_FLAGS_DEBUG "${KMPKG_COMBINED_C_FLAGS_DEBUG}")
    string(REGEX REPLACE "(^| )-Od( |$)" " " KMPKG_COMBINED_C_FLAGS_DEBUG "${KMPKG_COMBINED_C_FLAGS_DEBUG}")
    string(REGEX REPLACE "(^| )-Ob0( |$)" " " KMPKG_COMBINED_C_FLAGS_DEBUG "${KMPKG_COMBINED_C_FLAGS_DEBUG}")
endif()

string(APPEND KMPKG_COMBINED_C_FLAGS_DEBUG " -I \"${CURRENT_INSTALLED_DIR}/include\"")
string(APPEND KMPKG_COMBINED_C_FLAGS_RELEASE " -I \"${CURRENT_INSTALLED_DIR}/include\"")

## Setup kmpkg toolchain

set(prog_env "")

if(KMPKG_DETECTED_CMAKE_C_COMPILER)
    get_filename_component(CC_path "${KMPKG_DETECTED_CMAKE_C_COMPILER}" DIRECTORY)
    get_filename_component(CC_filename "${KMPKG_DETECTED_CMAKE_C_COMPILER}" NAME)
...
if(KMPKG_DETECTED_CMAKE_CXX_COMPILER)
    get_filename_component(CXX_path "${KMPKG_DETECTED_CMAKE_CXX_COMPILER}" DIRECTORY)
    get_filename_component(CXX_filename "${KMPKG_DETECTED_CMAKE_CXX_COMPILER}" NAME)
...
if(KMPKG_DETECTED_CMAKE_RC_COMPILER)
    get_filename_component(RC_path "${KMPKG_DETECTED_CMAKE_RC_COMPILER}" DIRECTORY)
    get_filename_component(RC_filename "${KMPKG_DETECTED_CMAKE_RC_COMPILER}" NAME)
    set(ENV{WINDRES} "${RC_filename}")
    string(APPEND OPTIONS " --windres=${RC_filename}")
    list(APPEND prog_env "${RC_path}")
endif()

if(KMPKG_DETECTED_CMAKE_LINKER AND KMPKG_TARGET_IS_WINDOWS AND NOT KMPKG_TARGET_IS_MINGW)
    get_filename_component(LD_path "${KMPKG_DETECTED_CMAKE_LINKER}" DIRECTORY)
    get_filename_component(LD_filename "${KMPKG_DETECTED_CMAKE_LINKER}" NAME)
...
if(KMPKG_DETECTED_CMAKE_NM)
    get_filename_component(NM_path "${KMPKG_DETECTED_CMAKE_NM}" DIRECTORY)
    get_filename_component(NM_filename "${KMPKG_DETECTED_CMAKE_NM}" NAME)
    set(ENV{NM} "${NM_filename}")
    string(APPEND OPTIONS " --nm=${NM_filename}")
    list(APPEND prog_env "${NM_path}")
endif()

if(KMPKG_DETECTED_CMAKE_AR)
    get_filename_component(AR_path "${KMPKG_DETECTED_CMAKE_AR}" DIRECTORY)
    get_filename_component(AR_filename "${KMPKG_DETECTED_CMAKE_AR}" NAME)
...
if(KMPKG_DETECTED_CMAKE_RANLIB)
    get_filename_component(RANLIB_path "${KMPKG_DETECTED_CMAKE_RANLIB}" DIRECTORY)
    get_filename_component(RANLIB_filename "${KMPKG_DETECTED_CMAKE_RANLIB}" NAME)
    set(ENV{RANLIB} "${RANLIB_filename}")
    string(APPEND OPTIONS " --ranlib=${RANLIB_filename}")
    list(APPEND prog_env "${RANLIB_path}")
endif()

if(KMPKG_DETECTED_CMAKE_STRIP)
    get_filename_component(STRIP_path "${KMPKG_DETECTED_CMAKE_STRIP}" DIRECTORY)
    get_filename_component(STRIP_filename "${KMPKG_DETECTED_CMAKE_STRIP}" NAME)
...
if(KMPKG_HOST_IS_WINDOWS)
    kmpkg_acquire_msys(MSYS_ROOT PACKAGES automake)
...
kmpkg_add_to_path(PREPEND ${prog_env})
...
if("openssl" IN_LIST FEATURES)
    ...
    if(KMPKG_TARGET_IS_WINDOWS AND NOT KMPKG_TARGET_IS_UWP)
...
if(KMPKG_TARGET_ARCHITECTURE STREQUAL "x64")
    ...
    set(BUILD_ARCH ${KMPKG_TARGET_ARCHITECTURE})
endif()
...
if (KMPKG_TARGET_IS_IOS)
    set(kmpkg_target_arch "${KMPKG_TARGET_ARCHITECTURE}")
    if (KMPKG_TARGET_ARCHITECTURE STREQUAL "x64")
        set(kmpkg_target_arch "x86_64")
    ...
    set(kmpkg_osx_deployment_target "${KMPKG_OSX_DEPLOYMENT_TARGET}")
    if (NOT KMPKG_OSX_DEPLOYMENT_TARGET)
        set(kmpkg_osx_deployment_target 11.0)
    elseif (KMPKG_OSX_DEPLOYMENT_TARGET LESS 11.0) # nowadays ffmpeg needs to be built for ios 11.0 and later
        message(FATAL_ERROR "ffmpeg can be built only for iOS 11.0 and later but you set KMPKG_OSX_DEPLOYMENT_TARGET to
                            ${KMPKG_OSX_DEPLOYMENT_TARGET}")
    endif ()

    if (KMPKG_OSX_SYSROOT STREQUAL "iphonesimulator")
        set(simulator "-simulator")
    endif ()

    set(OPTIONS "${OPTIONS} --extra-cflags=--target=${kmpkg_target_arch}-apple-ios${kmpkg_osx_deployment_target}${simulator}")
    set(OPTIONS "${OPTIONS} --extra-ldflags=--target=${kmpkg_target_arch}-apple-ios${kmpkg_osx_deployment_target}${simulator}")

    set(kmpkg_osx_sysroot "${KMPKG_OSX_SYSROOT}")
    ...
    if ((KMPKG_OSX_SYSROOT MATCHES "^(iphoneos|iphonesimulator)$") OR (NOT KMPKG_OSX_SYSROOT) OR (KMPKG_OSX_SYSROOT STREQUAL "")) # if it's not a path
        if (KMPKG_OSX_SYSROOT MATCHES "^(iphoneos|iphonesimulator)$")
            set(requested_sysroot "${KMPKG_OSX_SYSROOT}")
        elseif (KMPKG_TARGET_ARCHITECTURE STREQUAL "arm64") # if the sysroot is not specified we have to guess
            set(requested_sysroot "iphoneos")
        elseif (KMPKG_TARGET_ARCHITECTURE STREQUAL "x64")
            set(requested_sysroot "iphonesimulator")
        else ()
            message(FATAL_ERROR "Unsupported build arch: ${KMPKG_TARGET_ARCHITECTURE}")
        endif ()
        ...
        if (sdk_path)
            ...
            set(kmpkg_osx_sysroot "${sdk_path}")
        else ()
            message(FATAL_ERROR "Can't determine ${CMAKE_OSX_SYSROOT} SDK path. Error: ${xcrun_error}")
        endif ()
    endif ()
    set(OPTIONS "${OPTIONS} --extra-cflags=-isysroot\"${kmpkg_osx_sysroot}\"")
    set(OPTIONS "${OPTIONS} --extra-ldflags=-isysroot\"${kmpkg_osx_sysroot}\"")
endif ()
...
separate_arguments(standard_libraries NATIVE_COMMAND "${KMPKG_DETECTED_CMAKE_C_STANDARD_LIBRARIES}")
...
if (NOT KMPKG_BUILD_TYPE OR KMPKG_BUILD_TYPE STREQUAL "release")
    ...
    string(REGEX REPLACE "-arch [A-Za-z0-9_]+" "" KMPKG_COMBINED_C_FLAGS_RELEASE_SANITIZED "${KMPKG_COMBINED_C_FLAGS_RELEASE}")
    file(WRITE "${crsp}" "${KMPKG_COMBINED_C_FLAGS_RELEASE_SANITIZED}")
    set(ldrsp "${CURRENT_BUILDTREES_DIR}/${TARGET_TRIPLET}-rel/ldflags.rsp")
    string(REGEX REPLACE "-arch [A-Za-z0-9_]+" "" KMPKG_COMBINED_SHARED_LINKER_FLAGS_RELEASE_SANITIZED "${KMPKG_COMBINED_SHARED_LINKER_FLAGS_RELEASE}")
    file(WRITE "${ldrsp}" "${KMPKG_COMBINED_SHARED_LINKER_FLAGS_RELEASE_SANITIZED}")
    set(ENV{CFLAGS} "@${crsp}")
    ...
    if(NOT KMPKG_DETECTED_MSVC OR NOT KMPKG_TARGET_ARCHITECTURE MATCHES "^arm")
        set(ENV{ASFLAGS} "@${crsp}")
    endif()
    set(ENV{LDFLAGS} "@${ldrsp}")
    set(ENV{ARFLAGS} "${KMPKG_COMBINED_STATIC_LINKER_FLAGS_RELEASE}")
...
if (NOT KMPKG_BUILD_TYPE OR KMPKG_BUILD_TYPE STREQUAL "debug")
    ...
    string(REGEX REPLACE "-arch [A-Za-z0-9_]+" "" KMPKG_COMBINED_C_FLAGS_DEBUG_SANITIZED "${KMPKG_COMBINED_C_FLAGS_DEBUG}")
    file(WRITE "${crsp}" "${KMPKG_COMBINED_C_FLAGS_DEBUG_SANITIZED}")
    set(ldrsp "${CURRENT_BUILDTREES_DIR}/${TARGET_TRIPLET}-dbg/ldflags.rsp")
    string(REGEX REPLACE "-arch [A-Za-z0-9_]+" "" KMPKG_COMBINED_SHARED_LINKER_FLAGS_DEBUG_SANITIZED "${KMPKG_COMBINED_SHARED_LINKER_FLAGS_DEBUG}")
    file(WRITE "${ldrsp}" "${KMPKG_COMBINED_SHARED_LINKER_FLAGS_DEBUG_SANITIZED}")
    set(ENV{CFLAGS} "@${crsp}")
    if(NOT KMPKG_DETECTED_MSVC OR NOT KMPKG_TARGET_ARCHITECTURE MATCHES "^arm")
        set(ENV{ASFLAGS} "@${crsp}")
    endif()
    set(ENV{LDFLAGS} "@${ldrsp}")
    set(ENV{ARFLAGS} "${KMPKG_COMBINED_STATIC_LINKER_FLAGS_DEBUG}")
...
if(KMPKG_TARGET_IS_WINDOWS)
    ...
    if(NOT KMPKG_TARGET_IS_MINGW)
        ...
        foreach(DEF_FILE ${DEF_FILES})
            ...
            string(REGEX REPLACE "-[0-9]*\\.def" "${KMPKG_TARGET_STATIC_LIBRARY_SUFFIX}" OUT_FILE_NAME "${DEF_FILE_NAME}")
    ...
    file(GLOB LIB_FILES "${CURRENT_PACKAGES_DIR}/bin/*${KMPKG_TARGET_STATIC_LIBRARY_SUFFIX}" "${CURRENT_PACKAGES_DIR}/debug/bin/*${KMPKG_TARGET_STATIC_LIBRARY_SUFFIX}")
    if(KMPKG_TARGET_IS_MINGW)
...
if("ffmpeg" IN_LIST FEATURES)
    kmpkg_copy_tools(TOOL_NAMES ffmpeg AUTO_CLEAN)
endif()
if("ffprobe" IN_LIST FEATURES)
    kmpkg_copy_tools(TOOL_NAMES ffprobe AUTO_CLEAN)
endif()
if("ffplay" IN_LIST FEATURES)
    kmpkg_copy_tools(TOOL_NAMES ffplay AUTO_CLEAN)
endif()
...
x_kmpkg_pkgconfig_get_modules(PREFIX FFMPEG_PKGCONFIG MODULES ${FFMPEG_PKGCONFIG_MODULES} LIBS)
...
if(KMPKG_LIBRARY_LINKAGE STREQUAL "static" AND NOT KMPKG_TARGET_IS_WINDOWS AND NOT KMPKG_TARGET_IS_OSX AND NOT KMPKG_TARGET_IS_IOS)
...
kmpkg_install_copyright(FILE_LIST "${SOURCE_PATH}/${LICENSE_FILE}")
