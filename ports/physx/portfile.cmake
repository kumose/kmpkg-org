...
if(KMPKG_LIBRARY_LINKAGE STREQUAL "static")
    set(KMPKG_BUILD_STATIC_LIBS TRUE)
else()
    set(KMPKG_BUILD_STATIC_LIBS FALSE)
endif()
if(KMPKG_CRT_LINKAGE STREQUAL "static")
    set(KMPKG_LINK_CRT_STATICALLY TRUE)
else()
    set(KMPKG_LINK_CRT_STATICALLY FALSE)
endif()

# Adjust CMake options settings based on the target platform
if(KMPKG_TARGET_IS_LINUX AND KMPKG_TARGET_ARCHITECTURE STREQUAL "x64")
    set(PLATFORM_OPTIONS
        -DPX_BUILDSNIPPETS=OFF
        -DPX_BUILDPVDRUNTIME=OFF
        -DPX_GENERATE_STATIC_LIBRARIES=${KMPKG_BUILD_STATIC_LIBS}
    )
elseif(KMPKG_TARGET_IS_LINUX AND KMPKG_TARGET_ARCHITECTURE STREQUAL "arm64")
    set(PLATFORM_OPTIONS
        -DPX_BUILDSNIPPETS=OFF
        -DPX_BUILDPVDRUNTIME=OFF
        -DPX_GENERATE_STATIC_LIBRARIES=${KMPKG_BUILD_STATIC_LIBS}
    )
elseif(KMPKG_TARGET_IS_WINDOWS AND KMPKG_TARGET_ARCHITECTURE STREQUAL "x64")
    set(PLATFORM_OPTIONS
        -DPX_BUILDSNIPPETS=OFF
        -DPX_BUILDPVDRUNTIME=OFF
        -DPX_GENERATE_STATIC_LIBRARIES=${KMPKG_BUILD_STATIC_LIBS}
        -DNV_USE_STATIC_WINCRT=${KMPKG_LINK_CRT_STATICALLY}
...
if(KMPKG_TARGET_IS_WINDOWS)
    ...
    kmpkg_extract_source_archive(PHYSXGPU_SOURCE_PATH
        NO_REMOVE_ONE_LEVEL
        ARCHIVE "${ARCHIVE}"
...
if(KMPKG_TARGET_IS_LINUX AND KMPKG_TARGET_ARCHITECTURE STREQUAL "arm64")
...
elseif(KMPKG_TARGET_IS_LINUX AND KMPKG_TARGET_ARCHITECTURE STREQUAL "x64")
...
elseif(KMPKG_TARGET_IS_WINDOWS AND KMPKG_TARGET_ARCHITECTURE STREQUAL "x64") # Note: this will work for any Win64, default is vc17win64
...
kmpkg_cmake_configure(
    ...
    WINDOWS_USE_MSBUILD
    OPTIONS
        -DCMAKE_TOOLCHAIN_FILE=${KMPKG_CHAINLOAD_TOOLCHAIN_FILE}
        ${PLATFORM_OPTIONS}
...
# Compile and install in kmpkg's final installation directories all of the include headers and binaries for debug/release
kmpkg_cmake_install()

######################## Extract to final kmpkg install locations and fixup artifacts in wrong dirs ##############################

message("[PHYSX BUILD COMPLETED] Extracting build artifacts to kmpkg installation locations..")

# Artifacts paths are similar to <compiler>/<configuration>/[artifact] however kmpkg expects
...
function(copy_in_kmpkg_destination_folder_physx_artifacts)
...
copy_in_kmpkg_destination_folder_physx_artifacts(
    DIRECTORY "lib"
    SUFFIXES ${KMPKG_TARGET_STATIC_LIBRARY_SUFFIX} ${KMPKG_TARGET_IMPORT_LIBRARY_SUFFIX}
)
...
if(NOT KMPKG_LIBRARY_LINKAGE STREQUAL "static")
    ...
    copy_in_kmpkg_destination_folder_physx_artifacts(
        DIRECTORY "bin"
        SUFFIXES ${KMPKG_TARGET_SHARED_LIBRARY_SUFFIX} ".pdb"
    )
endif()
...
set(GPULIBNAMES "")
if(KMPKG_TARGET_IS_LINUX) # Both for arm and x64
...
kmpkg_cmake_config_fixup(PACKAGE_NAME unofficial-omniverse-physx-sdk
...
message("[KMPKG Omniverse PhysX port execution completed]")
