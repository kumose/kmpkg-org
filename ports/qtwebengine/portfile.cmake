...
set(qtwebengine_target "${KMPKG_TARGET_TRIPLET}-${KMPKG_CMAKE_SYSTEM_NAME}")
if(KMPKG_CROSSCOMPILING)
    ...
    file(WRITE "${CURRENT_PACKAGES_DIR}/share/${PORT}/kmpkg-port-config.cmake" "set(qtwebengine_host \"${qtwebengine_target}\")\n")
    if(FEATURES STREQUAL "core")
...
if(KMPKG_TARGET_IS_OSX AND "spellchecker" IN_LIST FEATURES)
...
if(NOT KMPKG_TARGET_IS_WINDOWS)
    ...
    if(NOT KMPKG_TARGET_IS_IOS AND NOT KMPKG_TARGET_IS_OSX)
    ...
    endforeach()
    # kmpkg ports exist, but don't work with chromium
    ...
    list(APPEND FEATURE_OPTIONS "-DKMPKG_LOCK_FIND_PACKAGE_PkgConfig=ON")
...
#kmpkg_find_acquire_program(GN) # Qt builds its own internal version
...
kmpkg_add_to_path(PREPEND "${NODEJS_DIR}")
...
kmpkg_add_to_path(APPEND "${PYTHON_DIR}")

kmpkg_add_to_path(PREPEND "${CURRENT_HOST_INSTALLED_DIR}/tools/gperf")
set(GPERF "${CURRENT_HOST_INSTALLED_DIR}/tools/gperf/gperf${KMPKG_HOST_EXECUTABLE_SUFFIX}")

if(CMAKE_HOST_WIN32) # WIN32 HOST probably has win_flex and win_bison!
    if(NOT EXISTS "${FLEX_DIR}/flex${KMPKG_HOST_EXECUTABLE_SUFFIX}")
        file(CREATE_LINK "${FLEX}" "${FLEX_DIR}/flex${KMPKG_HOST_EXECUTABLE_SUFFIX}")
    endif()
    if(NOT EXISTS "${BISON_DIR}/BISON${KMPKG_HOST_EXECUTABLE_SUFFIX}")
        file(CREATE_LINK "${BISON}" "${BISON_DIR}/bison${KMPKG_HOST_EXECUTABLE_SUFFIX}")
    endif()
endif()
...
if(buildtree_length GREATER 22 AND KMPKG_TARGET_IS_WINDOWS AND NOT KMPKG_TARGET_ARCHITECTURE MATCHES "arm64")
    message(WARNING "Buildtree path '${CURRENT_BUILDTREES_DIR}' is too long.\nConsider passing --x-buildtrees-root=<shortpath> to kmpkg!\nTrying to use '${CURRENT_BUILDTREES_DIR}/../tmp'")
    ...
    if(buildtree_length_new GREATER 22)
         message(FATAL_ERROR "Buildtree path is too long. Build will fail! Pass --x-buildtrees-root=<shortpath> to kmpkg!")
    endif()
    file(MAKE_DIRECTORY "${CURRENT_BUILDTREES_DIR}")
endif()

set(ENV{QTWEBENGINE_GN_THREADS} "${KMPKG_CONCURRENCY}")
set(ENV{NINJAFLAGS} "-j${KMPKG_CONCURRENCY} $ENV{NINJAFLAGS}")
...
qt_cmake_configure(
    ...
    OPTIONS
        ...
        -DKMPKG_LOCK_FIND_PACKAGE_BISON=ON
        -DKMPKG_LOCK_FIND_PACKAGE_FLEX=ON
        -DKMPKG_LOCK_FIND_PACKAGE_GPerf=ON
        -DKMPKG_LOCK_FIND_PACKAGE_Ninja=ON
        -DKMPKG_LOCK_FIND_PACKAGE_Nodejs=ON
...
if(NOT KMPKG_BUILD_TYPE)
    ...
    set(KMPKG_BUILD_TYPE debug)
    if(KMPKG_TARGET_IS_LINUX AND EXISTS "${CURRENT_BUILDTREES_DIR}/${TARGET_TRIPLET}-dbg/src/core/Debug/${target_args_gn}")
    ...
    kmpkg_host_path_list(PREPEND ENV{PKG_CONFIG_PATH} "${CURRENT_INSTALLED_DIR}/debug/lib/pkgconfig" "${CURRENT_INSTALLED_DIR}/share/pkgconfig")
...
block(SCOPE_FOR VARIABLES)
set(KMPKG_BUILD_TYPE release)
if(KMPKG_TARGET_IS_LINUX AND EXISTS "${CURRENT_BUILDTREES_DIR}/${TARGET_TRIPLET}-rel/src/core/Release/${target_args_gn}")
...
kmpkg_host_path_list(PREPEND ENV{PKG_CONFIG_PATH} "${CURRENT_INSTALLED_DIR}/lib/pkgconfig" "${CURRENT_INSTALLED_DIR}/share/pkgconfig")
kmpkg_cmake_install(ADD_BIN_TO_PATH)
endblock()
kmpkg_restore_env_variables(VARS PKG_CONFIG_PATH)

# Unroll response file references.
# Escape quotes in a way which survives kmpkg_cmake_config_fixup().
...
foreach(file IN LISTS cmake_target_files)
    ...
    while(haystack MATCHES "@([^>]*[.]rsp)")
        ...
        if(EXISTS "${response_file}")
            ...
            message("${Z_KMPKG_BACKCOMPAT_MESSAGE_LEVEL}" "No such response file: ${response_file}")
            set(replacement "")
        endif()
...
if("webengine" IN_LIST FEATURES)
    if(KMPKG_TARGET_IS_WINDOWS AND NOT KMPKG_BUILD_TYPE)
