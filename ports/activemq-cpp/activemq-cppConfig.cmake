...
macro(_activemq_cpp_windows_dependencies)
    ...
    if(ACTIVEMQ_CPP_LIBWS2 AND ACTIVEMQ_CPP_DLLWS2 AND ACTIVEMQ_CPP_LIBRPCRT4 AND ACTIVEMQ_CPP_DLLRPCRT4 AND ACTIVEMQ_CPP_LIBMSWSOCK AND ACTIVEMQ_CPP_DLLMSWSOCK)
        ...
        if(_ACTIVEMQ_CPP_MISSINGS_COUNT EQUAL 1)
            set(${CMAKE_FIND_PACKAGE_NAME}_NOT_FOUND_MESSAGE "Activemq-cpp kmpkg install dependency failure: Did not find windows dependency: ${_ACTIVEMQ_CPP_MISSINGS_STR}")
        else()
            set(${CMAKE_FIND_PACKAGE_NAME}_NOT_FOUND_MESSAGE "Activemq-cpp kmpkg install dependency failure: Did not find windows dependencies: ${_ACTIVEMQ_CPP_MISSINGS_STR}")
        endif()
...
# for kmpkg to as well, the config file has to cover the various products of
...
if (ACTIVEMQ_CPP_DLL_RELEASE)
    ...
    if (ACTIVEMQ_CPP_LIB_RELEASE AND ACTIVEMQ_CPP_DLL_DEBUG AND ACTIVEMQ_CPP_LIB_DEBUG)
        ...
        if (ACTIVEMQ_CPP_APR_LIB_RELEASE AND ACTIVEMQ_CPP_APR_DLL_RELEASE AND ACTIVEMQ_CPP_APR_LIB_DEBUG AND ACTIVEMQ_CPP_APR_DLL_DEBUG)
            ...
            set(${CMAKE_FIND_PACKAGE_NAME}_NOT_FOUND_MESSAGE "Activemq-cpp kmpkg install dependency failure: apr kmpkg port not found in ${_IMPORT_PREFIX}.")
            foreach(_MISSING 
        ...
        set(${CMAKE_FIND_PACKAGE_NAME}_NOT_FOUND_MESSAGE "Activemq-cpp kmpkg install error: Found ${_IMPORT_PREFIX}debug/bin/activemq-cppd.dll.")
        foreach(_MISSING 
    ...
    if (ACTIVEMQ_CPP_LIB_RELEASE)
        ...
        if (ACTIVEMQ_CPP_LIB_DEBUG)
            ...
            set(${CMAKE_FIND_PACKAGE_NAME}_NOT_FOUND_MESSAGE "Activemq-cpp kmpkg install error: Found ${ACTIVEMQ_CPP_LIB_RELEASE} but not ${_ACTIVEMQ_CPP_LIB_DEBUG_MISSING}.")
            set(${CMAKE_FIND_PACKAGE_NAME}_FOUND FALSE)
        endif()
    else()
        #
        ...
        if(ACTIVEMQ_CPP_LIB_RELEASE)
            ...
            if (ACTIVEMQ_CPP_LIB_DEBUG)
                ...
                if (ACTIVEMQ_CPP_APR_LIB_RELEASE AND ACTIVEMQ_CPP_APR_LIB_DEBUG)
                    ...
                    if (Threads_FOUND)
                        ...
                        set(${CMAKE_FIND_PACKAGE_NAME}_NOT_FOUND_MESSAGE "Activemq-cpp kmpkg install dependency failure: threads library not found.")
                        set(${CMAKE_FIND_PACKAGE_NAME}_FOUND FALSE)
                    endif()
                else()
                    set(${CMAKE_FIND_PACKAGE_NAME}_NOT_FOUND_MESSAGE "Activemq-cpp kmpkg install dependency failure: apr kmpkg port not found in ${_IMPORT_PREFIX}.")
            ...
            if (ACTIVEMQ_CPP_LIB_RELEASE)
                ...
                #
                set(${CMAKE_FIND_PACKAGE_NAME}_NOT_FOUND_MESSAGE "Activemq-cpp kmpkg unexpected install: could not find any expected activemq-cpp libraries under ${_IMPORT_PREFIX}. The CMake configuration file only understands Windows and Linux static and shared installs from kmpkg.")
                set(${CMAKE_FIND_PACKAGE_NAME}_FOUND FALSE)
            endif()
        endif()
    endif()
