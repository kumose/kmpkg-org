...
if(buildtrees_path_length GREATER 35 AND CMAKE_HOST_WIN32)
    kmpkg_buildpath_length_warning(35)
...
#set(KMPKG_BUILD_TYPE release) #You probably want to set this to reduce build type and space requirements
...
-- If ${PORT} fails post build validation please try manually reducing KMPKG_MAX_CONCURRENCY and open up an issue if it still cannot build. \n\
...
kmpkg_find_acquire_program(GPERF)
kmpkg_find_acquire_program(PYTHON3)
kmpkg_find_acquire_program(NINJA)
set(NODEJS "${CURRENT_HOST_INSTALLED_DIR}/tools/node/node${KMPKG_HOST_EXECUTABLE_SUFFIX}")
...
kmpkg_add_to_path(PREPEND "${GPERF_DIR}")
kmpkg_add_to_path(PREPEND "${NINJA_DIR}")
...
if(NOT KMPKG_TARGET_IS_WINDOWS)
    ...
    kmpkg_host_path_list(PREPEND ENV{PKG_CONFIG_PATH} "${CURRENT_INSTALLED_DIR}/lib/pkgconfig")
    kmpkg_host_path_list(PREPEND ENV{INCLUDE} "${CURRENT_INSTALLED_DIR}/include")
    kmpkg_host_path_list(PREPEND ENV{C_INCLUDE_PATH} "${CURRENT_INSTALLED_DIR}/include")
    kmpkg_host_path_list(PREPEND ENV{CPLUS_INCLUDE_PATH} "${CURRENT_INSTALLED_DIR}/include")
endif()
