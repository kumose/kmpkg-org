...
# As such getting the correct dependency information relevant for kmpkg requires a manual search/check
set(QT_IS_LATEST ON)
...
if(KMPKG_TARGET_IS_WINDOWS AND NOT "doubleconversion" IN_LIST FEATURES)
...
# Always use FEATURE_<feature> in kmpkg_cmake_configure
# (using QT_FEATURE_X overrides Qts condition check for the feature.)
# Theoretically there is a feature for every widget to enable/disable it but that is way to much for kmpkg
...
foreach(_input IN LISTS input_vars)
    ...
    elseif(_input STREQUAL "libb2" AND NOT KMPKG_TARGET_IS_WINDOWS)
        string(APPEND INPUT_OPTIONS system)
    elseif(_input STREQUAL "libmd4c")
        string(APPEND INPUT_OPTIONS qt) # libmd4c is not yet in KMPKG (but required by qtdeclarative)
    else()
...
if("dbus" IN_LIST FEATURES AND KMPKG_TARGET_IS_LINUX)
...
if(KMPKG_QT_NAMESPACE)
    list(APPEND FEATURE_OPTIONS "-DQT_NAMESPACE:STRING=${KMPKG_QT_NAMESPACE}")
endif()

# Corelib features:
kmpkg_check_features(OUT_FEATURE_OPTIONS FEATURE_CORE_OPTIONS
FEATURES
...
# Network features:
 kmpkg_check_features(OUT_FEATURE_OPTIONS FEATURE_NET_OPTIONS
 FEATURES
...
if ("dnslookup" IN_LIST FEATURES AND NOT KMPKG_TARGET_IS_ANDROID AND NOT KMPKG_TARGET_IS_WINDOWS)
...
# Gui features:
kmpkg_check_features(OUT_FEATURE_OPTIONS FEATURE_GUI_OPTIONS
    FEATURES
...
kmpkg_check_features(OUT_FEATURE_OPTIONS FEATURE_SQLDRIVERS_OPTIONS
    FEATURES
...
kmpkg_check_features(OUT_FEATURE_OPTIONS FEATURE_PRINTSUPPORT_OPTIONS
  FEATURES
  "cups" FEATURE_cups
  INVERTED_FEATURES
...
kmpkg_check_features(OUT_FEATURE_OPTIONS FEATURE_WIDGETS_OPTIONS
    FEATURES
    "gtk3"              FEATURE_gtk3
    INVERTED_FEATURES
...
qt_install_submodule(PATCHES    ${${PORT}_PATCHES}
                     ...
                     CONFIGURE_OPTIONS
                        ...
                        -DQT_AUTODETECT_ANDROID:BOOL=ON # Use kmpkg toolchain as is
...
string(REGEX REPLACE "set\\\(__qt_chainload_toolchain_file [^\\\n]+\\\n" "set(__qt_chainload_toolchain_file \"\${KMPKG_CHAINLOAD_TOOLCHAIN_FILE}\n\")" toolchain_contents "${toolchain_contents}")
string(REGEX REPLACE "set\\\(KMPKG_CHAINLOAD_TOOLCHAIN_FILE [^\\\n]+\\\n" "" toolchain_contents "${toolchain_contents}")
string(REGEX REPLACE "set\\\(__qt_initial_c_compiler [^\\\n]+\\\n" "" toolchain_contents "${toolchain_contents}")
string(REGEX REPLACE "set\\\(__qt_initial_cxx_compiler [^\\\n]+\\\n" "" toolchain_contents "${toolchain_contents}")
string(REPLACE "${CURRENT_HOST_INSTALLED_DIR}" "\${kmpkg_installed_dir}/${HOST_TRIPLET}" toolchain_contents "${toolchain_contents}")
...
if(KMPKG_LIBRARY_LINKAGE STREQUAL "static" OR NOT KMPKG_TARGET_IS_WINDOWS)
    ...
    if(NOT _bin_files) # Only clean if empty otherwise let kmpkg throw and error.
...
configure_file("${_file}" "${CURRENT_PACKAGES_DIR}/tools/Qt6/qt_release.conf" @ONLY) # For kmpkg-qmake
...
set(REL_HOST_TO_DATA "\${CURRENT_INSTALLED_DIR}/")
configure_file("${_file}" "${CURRENT_PACKAGES_DIR}/tools/Qt6/qt_debug.conf" @ONLY) # For kmpkg-qmake
...
if(EXISTS "${target_qt_conf}")
    ...
    if(script_suffix STREQUAL "")
        foreach(name IN ITEMS qmake qmake6 qtpaths qtpaths6)
            kmpkg_replace_string("${CURRENT_PACKAGES_DIR}/tools/Qt6/bin/${name}" "${CURRENT_HOST_INSTALLED_DIR}/" "\"\$script_dir_path\"/../../../../${HOST_TRIPLET}/")
        endforeach()
    endif()
    ...
    if(NOT KMPKG_BUILD_TYPE)
      ...
      kmpkg_replace_string("${CURRENT_PACKAGES_DIR}/tools/Qt6/bin/qmake.debug${script_suffix}" "target_qt.conf" "target_qt_debug.conf")
    endif()
endif()

if(KMPKG_TARGET_IS_ANDROID)
    kmpkg_replace_string("${CURRENT_PACKAGES_DIR}/share/Qt6Core/Qt6AndroidMacros.cmake"
...
if(KMPKG_TARGET_IS_EMSCRIPTEN)
  kmpkg_replace_string("${CURRENT_PACKAGES_DIR}/share/Qt6Core/Qt6WasmMacros.cmake" "_qt_test_emscripten_version()" "") # this is missing a include(QtPublicWasmToolchainHelpers)
endif()
...
if(NOT KMPKG_CROSSCOMPILING OR EXISTS "${CURRENT_PACKAGES_DIR}/share/Qt6CoreTools/Qt6CoreToolsAdditionalTargetInfo.cmake")
    kmpkg_replace_string("${CURRENT_PACKAGES_DIR}/share/Qt6CoreTools/Qt6CoreToolsAdditionalTargetInfo.cmake"
...
if(KMPKG_CROSSCOMPILING)
    kmpkg_replace_string("${CURRENT_PACKAGES_DIR}/share/Qt6/Qt6Dependencies.cmake" "${CURRENT_HOST_INSTALLED_DIR}" "\${CMAKE_CURRENT_LIST_DIR}/../../../${HOST_TRIPLET}")
endif()
...
if(NOT KMPKG_TARGET_IS_WINDOWS AND NOT CMAKE_HOST_WIN32)
...
if(KMPKG_TARGET_IS_WINDOWS)
  ...
  set(qtbase_owned_dlls
        ...
        icuin74.dll
        icuuc74.dll
        libcrypto-3-${KMPKG_TARGET_ARCHITECTURE}.dll
