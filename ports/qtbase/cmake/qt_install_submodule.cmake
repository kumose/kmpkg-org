...
include("${CURRENT_HOST_INSTALLED_DIR}/share/kmpkg-cmake/kmpkg-port-config.cmake")
include("${CURRENT_HOST_INSTALLED_DIR}/share/kmpkg-cmake-config/kmpkg-port-config.cmake")
...
function(qt_cmake_configure)
    ...
    kmpkg_find_acquire_program(PERL) # Perl is probably required by all qt ports for syncqt
    ...
    if(NOT PORT STREQUAL "qtwebengine" OR QT_IS_LATEST) # qtwebengine requires python2; since 6.3 python3
        kmpkg_find_acquire_program(PYTHON3) # Python is required by some qt ports
        get_filename_component(PYTHON3_PATH ${PYTHON3} DIRECTORY)
        kmpkg_add_to_path(${PYTHON3_PATH})
    endif()
    ...
    if(NOT DEFINED KMPKG_OSX_DEPLOYMENT_TARGET)
...
function(qt_fixup_and_cleanup)
    ...
    foreach(_comp IN LISTS COMPONENTS)
        if(EXISTS "${CURRENT_PACKAGES_DIR}/share/Qt6${_comp}")
            kmpkg_cmake_config_fixup(PACKAGE_NAME "Qt6${_comp}" CONFIG_PATH "share/Qt6${_comp}" TOOLS_PATH "tools/Qt6/bin")
            # Would rather put it into share/cmake as before but the import_prefix correction in kmpkg_cmake_config_fixup is working against that.
        else()
            message(STATUS "WARNING: Qt component ${_comp} not found/built!")
        endif()
    endforeach()
    #fix debug plugin paths (should probably be fixed in kmpkg_cmake_config_fixup)
    ...
    foreach(_debug_target IN LISTS DEBUG_CMAKE_TARGETS)
        kmpkg_replace_string("${_debug_target}" "{_IMPORT_PREFIX}/${qt_plugindir}" "{_IMPORT_PREFIX}/debug/${qt_plugindir}" IGNORE_UNCHANGED)
        kmpkg_replace_string("${_debug_target}" "{_IMPORT_PREFIX}/${qt_qmldir}" "{_IMPORT_PREFIX}/debug/${qt_qmldir}" IGNORE_UNCHANGED)
    endforeach()
    ...
    foreach(_plugin_target IN LISTS STATIC_CMAKE_TARGETS)
        # restore a single get_filename_component which was remove by kmpkg_cmake_config_fixup
        kmpkg_replace_string("${_plugin_target}"
    ...
    foreach(_tool IN LISTS _qarg_TOOL_NAMES)
        if(NOT EXISTS "${CURRENT_PACKAGES_DIR}/bin/${_tool}${KMPKG_TARGET_EXECUTABLE_SUFFIX}")
    ...
    if(_qarg_TOOL_NAMES)
        ...
        kmpkg_copy_tools(TOOL_NAMES ${tool_names} SEARCH_DIR "${qt_searchdir}" DESTINATION "${qt_tooldest}" AUTO_CLEAN)
    endif()
    ...
    if(KMPKG_LIBRARY_LINKAGE STREQUAL "static")
        ...
        else() # Only clean if empty otherwise let kmpkg throw and error.
