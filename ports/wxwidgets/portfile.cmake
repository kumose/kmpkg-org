...
set(OPTIONS "")
if(KMPKG_TARGET_IS_WINDOWS AND (KMPKG_TARGET_ARCHITECTURE STREQUAL "arm64" OR KMPKG_TARGET_ARCHITECTURE STREQUAL "arm"))
    list(APPEND OPTIONS
...
if("webview" IN_LIST FEATURES AND KMPKG_LIBRARY_LINKAGE STREQUAL "static")
...
kmpkg_cmake_install()
kmpkg_cmake_config_fixup(CONFIG_PATH lib/cmake/wxWidgets)
...
if(NOT EXISTS "${CURRENT_PACKAGES_DIR}/include/wx/setup.h")
    ...
    if(NOT DEFINED KMPKG_BUILD_TYPE OR KMPKG_BUILD_TYPE STREQUAL "release")
        kmpkg_replace_string("${WX_SETUP_H_FILES_REL}" "${CURRENT_PACKAGES_DIR}" "" IGNORE_UNCHANGED)
    ...
    if(NOT DEFINED KMPKG_BUILD_TYPE OR KMPKG_BUILD_TYPE STREQUAL "debug")
        kmpkg_replace_string("${WX_SETUP_H_FILES_DBG}" "${CURRENT_PACKAGES_DIR}" "" IGNORE_UNCHANGED)
...
foreach(config IN LISTS configs)
    kmpkg_replace_string("${config}" "${CURRENT_INSTALLED_DIR}" [[${prefix}]])
endforeach()
...
foreach(config IN LISTS configs)
    kmpkg_replace_string("${config}" "${CURRENT_INSTALLED_DIR}/debug" [[${prefix}]])
endforeach()
...
if(NOT "debug-support" IN_LIST FEATURES)
    if(KMPKG_TARGET_IS_WINDOWS)
        kmpkg_replace_string("${CURRENT_PACKAGES_DIR}/include/wx/debug.h" "#define wxDEBUG_LEVEL 1" "#define wxDEBUG_LEVEL 0")
    else()
        kmpkg_replace_string("${CURRENT_PACKAGES_DIR}/include/wx-3.3/wx/debug.h" "#define wxDEBUG_LEVEL 1" "#define wxDEBUG_LEVEL 0")
    endif()
endif()
...
if("example" IN_LIST FEATURES)
    ...
    kmpkg_replace_string("${CURRENT_PACKAGES_DIR}/share/${PORT}/example/popup.cpp" "../sample.xpm" "sample.xpm")
endif()
...
kmpkg_install_copyright(FILE_LIST "${SOURCE_PATH}/docs/licence.txt")
