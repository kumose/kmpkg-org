...
# With kmpkg_from_gitlab, we modify `versions.sh` accordingly.
# For --editable mode, use configured patch instead of kmpkg_replace_string.
...
function(add_cross_prefix)
  if(configure_env MATCHES "CC=([^\/]*-)gcc$")
      kmpkg_list(APPEND arg_OPTIONS "--cross-prefix=${CMAKE_MATCH_1}")
  endif()
...
if(NOT "asm" IN_LIST FEATURES)
    kmpkg_list(APPEND OPTIONS --disable-asm)
elseif(NOT "$ENV{AS}" STREQUAL "")
    # Accept setting from triplet
elseif(KMPKG_TARGET_ARCHITECTURE IN_LIST nasm_archs)
    kmpkg_find_acquire_program(NASM)
    kmpkg_insert_program_into_path("${NASM}")
    set(ENV{AS} "${NASM}")
elseif(KMPKG_TARGET_ARCHITECTURE IN_LIST gaspp_archs AND KMPKG_TARGET_IS_WINDOWS AND KMPKG_HOST_IS_WINDOWS)
...
if("tool" IN_LIST FEATURES)
    kmpkg_list(APPEND OPTIONS_RELEASE --enable-cli)
else()
    kmpkg_list(APPEND OPTIONS_RELEASE --disable-cli)
endif()

if("chroma-format-all" IN_LIST FEATURES)
    kmpkg_list(APPEND OPTIONS --chroma-format=all)
endif()

if(NOT "gpl" IN_LIST FEATURES)
    kmpkg_list(APPEND OPTIONS --disable-gpl)
endif()
...
if("tool" IN_LIST FEATURES)
    kmpkg_copy_tools(TOOL_NAMES x264 AUTO_CLEAN)
endif()

file(REMOVE_RECURSE "${CURRENT_PACKAGES_DIR}/debug/include")

if(KMPKG_TARGET_IS_WINDOWS AND NOT KMPKG_TARGET_IS_MINGW)
    kmpkg_replace_string("${CURRENT_PACKAGES_DIR}/lib/pkgconfig/x264.pc" "-lx264" "-llibx264")
    if(NOT KMPKG_BUILD_TYPE)
        kmpkg_replace_string("${CURRENT_PACKAGES_DIR}/debug/lib/pkgconfig/x264.pc" "-lx264" "-llibx264")
    endif()
endif()

if(KMPKG_LIBRARY_LINKAGE STREQUAL "dynamic" AND KMPKG_TARGET_IS_WINDOWS AND NOT KMPKG_TARGET_IS_MINGW)
    ...
    kmpkg_replace_string("${CURRENT_PACKAGES_DIR}/include/x264.h" "#ifdef X264_API_IMPORTS" "#if 1")
elseif(KMPKG_LIBRARY_LINKAGE STREQUAL "static")
    kmpkg_replace_string("${CURRENT_PACKAGES_DIR}/include/x264.h" "defined(U_STATIC_IMPLEMENTATION)" "1" IGNORE_UNCHANGED)
    file(REMOVE_RECURSE
