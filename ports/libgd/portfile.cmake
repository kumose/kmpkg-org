...
if(BUILD_STATIC)
    kmpkg_replace_string("${CURRENT_PACKAGES_DIR}/include/gd.h" "ifdef NONDLL" "if 1")
endif()
if(KMPKG_TARGET_IS_WINDOWS AND NOT KMPKG_TARGET_IS_MINGW)
    string(REPLACE "_dynamic" "" suffix "_${KMPKG_LIBRARY_LINKAGE}")
    kmpkg_replace_string("${CURRENT_PACKAGES_DIR}/lib/pkgconfig/gdlib.pc" " -lgd" " -llibgd${suffix}")
    if(NOT KMPKG_BUILD_TYPE)
        kmpkg_replace_string("${CURRENT_PACKAGES_DIR}/debug/lib/pkgconfig/gdlib.pc" " -lgd" " -llibgd${suffix}")
    endif()
endif()
...
if(ENABLE_TOOLS)
    ...
    kmpkg_list(SET tool_names gdcmpgif)
    if(ENABLE_PNG)
        kmpkg_list(APPEND tool_names gdtopng pngtogd)
    endif()
    if(NOT KMPKG_TARGET_IS_WINDOWS)
        if(ENABLE_FREETYPE AND ENABLE_JPEG)
            kmpkg_list(APPEND tool_names annotate)
        endif()
        if(ENABLE_PNG)
            kmpkg_list(APPEND tool_names webpng)
        endif()
    endif()
