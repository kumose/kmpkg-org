...
kmpkg_add_to_path("${CURRENT_HOST_INSTALLED_DIR}/tools/python3") # port ask python distutils for info.
if(KMPKG_TARGET_IS_WINDOWS AND NOT KMPKG_TARGET_IS_MINGW)
  set(ENV{PYTHONPATH} "${CURRENT_HOST_INSTALLED_DIR}/tools/python3/Lib${KMPKG_HOST_PATH_SEPARATOR}${CURRENT_BUILDTREES_DIR}/${TARGET_TRIPLET}-rel/lib/python${KMPKG_HOST_PATH_SEPARATOR}${CURRENT_BUILDTREES_DIR}/${TARGET_TRIPLET}-dbg/lib/python")
endif()
...
if(KMPKG_TARGET_IS_WINDOWS AND NOT KMPKG_TARGET_IS_MINGW)
  set(z_kmpkg_org_linkage "${KMPKG_LIBRARY_LINKAGE}") 
  ...
  list(TRANSFORM progs PREPEND "KMPKG_DETECTED_CMAKE_")
  ...
  endforeach()
  configure_file("${CMAKE_CURRENT_LIST_DIR}/kmpkg.mk" "${SOURCE_PATH}/mk/platforms/kmpkg.mk" @ONLY NEWLINE_STYLE UNIX)
...
if(KMPKG_TARGET_IS_WINDOWS AND NOT KMPKG_TARGET_IS_MINGW)
  kmpkg_replace_string("${CURRENT_BUILDTREES_DIR}/${TARGET_TRIPLET}-rel//mk/platforms/kmpkg.mk" "replace-with-per-config-text" "NoDebugBuild=1")
  if(NOT KMPKG_BUILD_TYPE)
    kmpkg_replace_string("${CURRENT_BUILDTREES_DIR}/${TARGET_TRIPLET}-dbg/mk/platforms/kmpkg.mk" "replace-with-per-config-text" "NoReleaseBuild=1\nBuildDebugBinary=1")
    kmpkg_replace_string("${CURRENT_BUILDTREES_DIR}/${TARGET_TRIPLET}-dbg/src/tool/omniidl/cxx/dir.mk" "python$(subst .,,$(PYVERSION)).lib" "python$(subst .,,$(PYVERSION))_d.lib")
    kmpkg_replace_string("${CURRENT_BUILDTREES_DIR}/${TARGET_TRIPLET}-dbg/src/tool/omniidl/cxx/dir.mk" "zlib.lib" "zlibd.lib")
  endif()
endif()
...
if(KMPKG_TARGET_IS_WINDOWS AND NOT KMPKG_TARGET_IS_MINGW)
  ...
  if(z_kmpkg_org_linkage STREQUAL "static")
    ...
    foreach(pc_file IN LISTS pc_files)
      ...
      if(filename STREQUAL "omnithread3")
        kmpkg_replace_string("${pc_file}" "-lomnithread" "-lomnithread_rt")
      else()
        kmpkg_replace_string("${pc_file}" "-l${filename}" "-l${filename}_rt" IGNORE_UNCHANGED)
      endif()
    endforeach()
  endif()

  if(NOT KMPKG_BUILD_TYPE) # dbg libs have no install rules so manually copy them.
    ...
    if(z_kmpkg_org_linkage STREQUAL "static")
      ...
      foreach(pc_file IN LISTS pc_files)
        get_filename_component(filename "${pc_file}" NAME_WE)
        if(filename STREQUAL "omnithread3")
          kmpkg_replace_string("${pc_file}" "-lomnithread" "-lomnithread_rtd")
        else()
          kmpkg_replace_string("${pc_file}" "-l${filename}" "-l${filename}_rtd" IGNORE_UNCHANGED)
        endif()
      endforeach()
    endif()
...
kmpkg_install_copyright(FILE_LIST "${SOURCE_PATH}/COPYING.LIB") # Lib is LGPL
...
if(KMPKG_TARGET_IS_WINDOWS AND NOT KMPKG_TARGET_IS_MINGW)
file(COPY
      "${CURRENT_BUILDTREES_DIR}/${TARGET_TRIPLET}-rel/bin/omnicpp${KMPKG_TARGET_EXECUTABLE_SUFFIX}"
    DESTINATION
...
file(COPY
      "${CURRENT_BUILDTREES_DIR}/${TARGET_TRIPLET}-rel/bin/omniidl${KMPKG_TARGET_EXECUTABLE_SUFFIX}"
    DESTINATION
      "${CURRENT_PACKAGES_DIR}/tools/${PORT}/bin"
    )
...
if(KMPKG_TARGET_IS_WINDOWS AND NOT KMPKG_TARGET_IS_MINGW)
   set(KMPKG_LIBRARY_LINKAGE "${z_kmpkg_org_linkage}")
endif()

if(NOT KMPKG_TARGET_IS_WINDOWS)
  kmpkg_replace_string("${CURRENT_PACKAGES_DIR}/tools/${PORT}/bin/omniidl" "${CURRENT_INSTALLED_DIR}" "\"os.path.dirname(__file__)+\"/../../../")
endif()
