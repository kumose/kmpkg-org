...
kmpkg_check_features(OUT_FEATURE_OPTIONS FEATURE_OPTIONS
  FEATURES
    ...
    rocm    USE_ROCM  # This is an alternative to cuda not a feature! (Not in kmpkg.json!) -> disabled
    llvm    USE_LLVM
    mpi     USE_MPI
    nnpack  USE_NNPACK  # todo: check use of `DISABLE_NNPACK_AND_FAMILY`
#   No feature in kmpkg yet so disabled. -> Requires numpy build by kmpkg itself
    python  BUILD_PYTHON
    python  USE_NUMPY
    glog    USE_GLOG
    gflags  USE_GFLAGS
...
if("dist" IN_LIST FEATURES)
    ...
    list(APPEND FEATURE_OPTIONS -DUSE_GLOO=${KMPKG_TARGET_IS_LINUX})
endif()
...
if(KMPKG_TARGET_IS_ANDROID OR KMPKG_TARGET_IS_IOS)
    set(TARGET_IS_MOBILE ON)
endif()

set(TARGET_IS_APPLE OFF)
if(KMPKG_TARGET_IS_IOS OR KMPKG_TARGET_IS_OSX)
    set(TARGET_IS_APPLE ON)
endif()

string(COMPARE EQUAL "${KMPKG_CRT_LINKAGE}" "static" USE_STATIC_RUNTIME)
kmpkg_cmake_configure(
    ...
    OPTIONS
        ...
        -DUSE_NUMA=OFF
        -DUSE_SYSTEM_LIBS=ON
        -DBUILD_JNI=${KMPKG_TARGET_IS_ANDROID}
        -DUSE_NNAPI=${KMPKG_TARGET_IS_ANDROID}
        ${BLAS_OPTIONS}
...
kmpkg_cmake_config_fixup(PACKAGE_NAME Caffe2 CONFIG_PATH "share/cmake/Caffe2" DO_NOT_DELETE_PARENT_CONFIG_PATH)
kmpkg_cmake_config_fixup(PACKAGE_NAME torch CONFIG_PATH "share/cmake/Torch" DO_NOT_DELETE_PARENT_CONFIG_PATH)
kmpkg_cmake_config_fixup(PACKAGE_NAME ATen CONFIG_PATH "share/cmake/ATen" )

kmpkg_replace_string("${CURRENT_PACKAGES_DIR}/share/torch/TorchConfig.cmake" "/../../../" "/../../")
...
set(KMPKG_POLICY_DLLS_WITHOUT_EXPORTS enabled) # torch_global_deps.dll is empty.c and just for linking deps
