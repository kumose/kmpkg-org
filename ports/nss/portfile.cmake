...
elseif(KMPKG_TARGET_IS_OPENBSD)
...
if(KMPKG_TARGET_ARCHITECTURE STREQUAL "x86")
    ...
    list(APPEND OPTIONS "--target=${KMPKG_TARGET_ARCHITECTURE}")
endif()
...
if(CMAKE_HOST_WIN32 AND KMPKG_TARGET_IS_WINDOWS AND NOT KMPKG_TARGET_IS_MINGW)
    ...
    if(KMPKG_DETECTED_MSVC)
        ...
        if(NOT EXISTS "${vswhere}")
            kmpkg_execute_in_download_mode(
                COMMAND "$ENV{KMPKG_COMMAND}" fetch vswhere --x-stderr-status
        ...
        kmpkg_host_path_list(APPEND ENV{PATH} "${vswhere_dir}")
        ...
        if("$ENV{GYP_MSVS_OVERRIDE_PATH}" STREQUAL "" OR "$ENV{GYP_MSVS_VERSION}" STREQUAL "")
            execute_process(
                COMMAND "${vswhere}"
                    ...
                    -path "${KMPKG_DETECTED_CMAKE_C_COMPILER}"
                OUTPUT_VARIABLE msvs_installdir
                OUTPUT_STRIP_TRAILING_WHITESPACE
            )
            message(STATUS "MSVS resolvedInstallationPath: ${msvs_installdir}")
            if(NOT EXISTS "${msvs_installdir}")
                message(FATAL_ERROR "Failed to determine MSVS dir for ${KMPKG_DETECTED_CMAKE_C_COMPILER}.")
            endif()
            ...
            if(NOT msvs_version MATCHES "^20..e?\$")
                message(FATAL_ERROR "Failed to determine MSVS version for ${KMPKG_DETECTED_CMAKE_C_COMPILER}.")
            endif()
...
x_kmpkg_pkgconfig_get_modules(PREFIX PC_NSPR MODULES nspr CFLAGS LIBS)
x_kmpkg_pkgconfig_get_modules(PREFIX PC_SQLITE MODULES sqlite3 CFLAGS LIBS)
x_kmpkg_pkgconfig_get_modules(PREFIX PC_ZLIB MODULES zlib CFLAGS LIBS)
...
kmpkg_install_copyright(FILE_LIST "${SOURCE_PATH}/nss/COPYING")
