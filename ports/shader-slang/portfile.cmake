...
elseif(KMPKG_TARGET_IS_OSX)
	set(key "macosx-${KMPKG_TARGET_ARCHITECTURE}")
elseif(KMPKG_TARGET_IS_LINUX)
	set(key "linux-${KMPKG_TARGET_ARCHITECTURE}")
endif()

set(ARCHIVE NOTFOUND)
# For convenient updates, use 
# kmpkg install shader-slang --cmake-args=-DKMPKG_SHADER_SLANG_UPDATE=1
if(key STREQUAL "windows-x64" OR KMPKG_SHADER_SLANG_UPDATE)
...
if(key STREQUAL "windows-arm64" OR KMPKG_SHADER_SLANG_UPDATE)
	kmpkg_download_distfile(
		ARCHIVE
...
if(key STREQUAL "macosx-x64" OR KMPKG_SHADER_SLANG_UPDATE)
	kmpkg_download_distfile(
		ARCHIVE
...
if(key STREQUAL "macosx-arm64" OR KMPKG_SHADER_SLANG_UPDATE)
	kmpkg_download_distfile(
		ARCHIVE
...
if(key STREQUAL "linux-x64" OR KMPKG_SHADER_SLANG_UPDATE)
	kmpkg_download_distfile(
		ARCHIVE
...
if(key STREQUAL "linux-arm64" OR KMPKG_SHADER_SLANG_UPDATE)
	kmpkg_download_distfile(
		ARCHIVE
...
if(KMPKG_SHADER_SLANG_UPDATE)
	message(STATUS "All downloads are up-to-date.")
	message(FATAL_ERROR "Stopping due to KMPKG_SHADER_SLANG_UPDATE being enabled.")
endif()
...
# kmpkg wants to be helpful and update the rpath as it moves binaries around but this 
...
# Must manually copy some tool dependencies since kmpkg can't copy them automagically for us
file(INSTALL ${dyn_libs} DESTINATION "${CURRENT_PACKAGES_DIR}/tools/shader-slang")
kmpkg_copy_tools(TOOL_NAMES slangc slangd slangi SEARCH_DIR "${BINDIST_PATH}/bin")

file(GLOB headers "${BINDIST_PATH}/include/*.h")
file(INSTALL ${headers} DESTINATION "${CURRENT_PACKAGES_DIR}/include")

block(SCOPE_FOR VARIABLES)
	set(KMPKG_BUILD_TYPE Release) # no debug binaries anyways
	file(COPY "${BINDIST_PATH}/cmake" DESTINATION "${CURRENT_PACKAGES_DIR}")
	kmpkg_cmake_config_fixup(CONFIG_PATH cmake PACKAGE_NAME slang)
	kmpkg_replace_string("${CURRENT_PACKAGES_DIR}/share/slang/slangConfig.cmake"
		[[HINTS ENV PATH "${PACKAGE_PREFIX_DIR}/bin"]]
		[[PATHS "${PACKAGE_PREFIX_DIR}/tools/shader-slang" NO_DEFAULT_PATH REQUIRED]]
	)
	kmpkg_replace_string("${CURRENT_PACKAGES_DIR}/share/slang/slangConfigVersion.cmake"
